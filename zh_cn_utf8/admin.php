...
$string['allowuserblockhiding'] = '允许用户们隐藏模块';
...
$string['autologinguests'] = '自动登陆的访客';
...
$string['configallowassign'] = '允许左边的角色们分配每栏中的角色们';
...
$string['configallowoverride'] = '允许左边的角色们覆盖每栏中的角色们';
...
$string['configdefaultcourseroleid'] = '将会自动给注册此课程的用户分配该角色。';
...
$string['configenableajax'] = '此项设置允许您通过整个网站控制AJAX的使用（高级的客户/服务器界面使用Java描述语言）。依据此设置使得用户们可以始终在个人资料中进行选择，否则AJAX对每个人都是不能使用的。';
...
$string['configenabletrusttext'] = '默认情况下，moodle将始终彻底清除因用户们移除任何可能的坏脚本、媒介等可能据有安全隐患的内容而产生的的文本。职责内容系统是一种解决方法，用来给那些您信任的特别用户具有一些本身没有的高级特征的能力。为了使此系统有效，您需要首先使此设置有效，然后给指定的moodle角色授予职责内容许可。由此类用户们产生或上传的文本将被标记为可信任的，并且将在显示后不清除掉。';
...
$string['configgradebookroles'] = '此设置允许您控制出现在成绩簿上的人。用户们至少需要有此课程中的一种角色才会显示在成绩簿中。';
...
$string['confignotloggedinroleid'] = '未登陆此网站的用户们将被处理成像是有查看网站内容的角色。访客几乎始终是您在这里希望的那样，但是您可能希望产生的角色们或多或少受到限制。类似产生摘要这样的事情始终需要用户正确登录。';
...
$string['forcelogin'] = '强制用户们登陆';
$string['forceloginforprofiles'] = '强制用户们为资料登录';
...
$string['protectusernames'] = '保护用户们的名字';
...
$string['riskconfig'] = '用户们可以改变网站的设置和行为';
$string['riskmanagetrust'] = '用户们可以改变其它用户的职责设置';
$string['riskpersonal'] = '用户们可以获得访问其它用户私人信息的权利';
$string['riskspam'] = '用户们可以发送信息给站内用户或其它人';
$string['riskxss'] = '用户们可以添加允许跨网站描述的（XSS）文件和文本';
