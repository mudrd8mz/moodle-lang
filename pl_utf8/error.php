<?PHP // $Id$ 
      // error.php - created with Moodle 1.6 Beta 5 (2006050500)


$string['adminprimarynoedit'] = 'Ustawienia głównego administratora nie mogą być zmieniane przez innych';
$string['cannotcreatelangdir'] = 'Nie można utworzyć katelogu do \"paczki językowej\" (lang)';
$string['cannotcreatetempdir'] = 'Nie można utworzyć katalogu tymczasowego';
$string['cannotdownloadcomponents'] = 'Nie można pobrać składowych (komponentów)';
$string['cannotdownloadzipfile'] = 'Nie można pobrać spakowanego pliku (ZIP)';
$string['cannotfindcomponent'] = 'Nie można znaleźć składowej (komponentu)';
$string['cannotsavemd5file'] = 'Nie mozna zapisać pliku md5';
$string['cannotsavezipfile'] = 'Nie można zapisać spakowanego pliku ZIP';
$string['cannotunzipfile'] = 'Nie można rozpakować pliku ZIP';
$string['cmunknown'] = 'Nie można znaleźć modułu kursu';
$string['componentisuptodate'] = 'Składowa (komponent) jest aktualny';
$string['confirmsesskeybad'] = 'Niestety Twój klucz sesji nie pozwala wykonać tej operacji. Zabezpieczenie te zapobiega przypadkowym lub złośliwym wykonaniu operacji w Twoim imieniu. Zrób to jeżeli naprawdę musisz wykonać tę operację';
$string['coursegroupunknown'] = 'Nie określono kursu dla grupy $a';
$string['downloadedfilecheckfailed'] = 'Pobierany plik jest uszkodzony';
$string['errorcleaningdirectory'] = 'Błąd czyszczenia katalogu. \"$a\"';
$string['errorcopyingfiles'] = 'Błąd kopiowania plików';
$string['errorcreatingdirectory'] = 'Błąd tworzenia katalogu. \"$a\";';
$string['errorcreatingfile'] = 'Błąd tworzenia pliku \"$a\"';
$string['erroronline'] = 'Błąd na lini $a.';
$string['errorreadingfile'] = 'Błąd odczytu pliku\"$a\"';
$string['errorunzippingfiles'] = 'Błąd rozpakowywania plików';
$string['fieldrequired'] = '\"$a\" nie może być puste.';
$string['filenotfound'] = 'Niestety nie mogę znaleźć pliku';
$string['forumblockingtoomanyposts'] = 'Przekroczyłeś liczbę możliwych postów wysyłanych na te forum';
$string['groupalready'] = 'Użytkownik już przynależy do grupy $a';
$string['groupunknown'] = 'Grupa $a nie jest przypisana do tego kursu';
$string['invalidcourse'] = 'Nieważny kurs';
$string['invalidfieldname'] = '\"$a\" nie jest dozwoloną nazwą.';
$string['invalidfiletype'] = '\"$a\" nie jest obsługiwanym typem pliku';
$string['invalidxmlfile'] = '\"$a\" nie jest obsługiwanym plikiem XLM';
$string['missingfield'] = 'Brakuje pola \"$a\" .';
$string['modulemissingcode'] = 'Moduł $a stracił kod niezbędny do działania tej funkcji';
$string['modulerequirementsnotmet'] = 'Moduł \"$a->modulename\" ($a->moduleversion) nie został poprawnie zainstalowany. Zainstaluj nowszą wersję Moodle! Aktualnie używasz wersji $a->currentmoodle, a potrzebujesz $a->requiremoodle).';
$string['mustbeteacher'] = 'Dostęp do tej strony jedynie dla nauczyciela';
$string['noinstances'] = 'Tego typu przypadki nie występują $a w tym kursie!';
$string['nonmeaningfulcontent'] = 'Brak znaczącej treści.';
$string['noparticipatorycms'] = 'Przepraszam, lecz nie posiadasz modułów biorących udział w kursie odpowiedzialnych za możliwość zgłoszeń.';
$string['notavailable'] = 'Nie jest to w tej chwili dostępne';
$string['onlyeditingteachers'] = 'Przeznaczone jedynie dla nauczycieli posiadających zdolność edycji.';
$string['onlyeditown'] = 'Możesz edytować tylko własne informacje';
$string['processingstops'] = 'Operacja została zatrzymana. Operacja zapisu nie została wykonana';
$string['restricteduser'] = 'Twoje konto \"$a\" nie daje Tobie uprawnień do wykonania tej operacji.';
$string['sessionerroruser'] = 'Sesja wygasła. Zaloguj się ponownie.';
$string['sessionerroruser2'] = 'Błąd serwera, który oddziaływał na wykrycie Twojej sesji logowania. Proszę zalogować się ponownie lub zrestartować swoją przeglądarkę.';
$string['sessionipnomatch'] = 'Twój nr IP został zmieniony od kiedy zalogowałeś się po raz pierwszy. Zabezpieczenie te zapobiega wykorzystania przez osoby niepowołane Twojego identyfikatora do logowania się na tej stronie. Skontaktuj się z administratorem tej strony.';
$string['statscatchupmode'] = 'Statystyki znajdują się obecnie w trybie aktualizacji. Przez $a->daysdone dzni będą przetwarzane a przez $a->dayspending są oczekujące. Sprawdź ponownie wkrótce! ';
$string['unknowncourse'] = 'Kurs o nazwie \"$a\" nie istnieje.';
$string['usernotaddederror'] = 'Nieznany błąd - użytkownik \"$a\" nie został dodany.';
$string['usernotaddedregistered'] = 'Yżytkownik \"$a\" już istnieje.';
$string['usernotavailable'] = 'Szczegóły tego użytkownika nie są dla Ciebie dostępne';
$string['usernotrenamedexists'] = 'Użytkownik nie zmienił swojej nazwy – nowa nazwa użytkownika jest już używana.';
$string['usernotrenamedmissing'] = 'Użytkownik nie zmienił swojej nazwy – nie można znaleźć starej nazwy użytkownika.';

?>
