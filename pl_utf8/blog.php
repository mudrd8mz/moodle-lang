<?PHP // $Id$ 
      // blog.php - created with Moodle 1.6 Beta 5 (2006050500)


$string['addnewentry'] = 'Dodaj nowy wpis';
$string['addotags'] = 'Dodaj oficjalne znaczniki';
$string['addptags'] = 'Dodaj znaczniki definiowane przez użytkownika';
$string['blockmenutitle'] = 'Blog';
$string['blocktagstitle'] = 'Znaczniki bloga';
$string['blocktitle'] = 'Tytuł bloku znaczników bloga';
$string['blog'] = 'Blog';
$string['blogdeleteconfirm'] = 'Usunąć ten blog?';
$string['blogpreferences'] = 'Preferencje blogu';
$string['blogs'] = 'Blogi';
$string['blogtags'] = 'Znaczniki bloga';
$string['courseblog'] = 'Blog kursu: $a';
$string['courseblogs'] = 'Użytkownicy mogą wyświetlać tylko blogi dla osób współdzielących kurs';
$string['disableblogs'] = 'Wyłącz całkowicie system blogów';
$string['entrybody'] = 'Treść wpisu blogu';
$string['entrybodyonlydesc'] = 'Opis wpisu';
$string['entryerrornotyours'] = 'Ten wpis nie jest twój';
$string['entrytitle'] = 'Tytuł wpisu';
$string['entryupdated'] = 'Uaktualniono wpis blogu';
$string['groupblog'] = 'Blog grupy: $a';
$string['groupblogs'] = 'Użytkownicy mogą wyświetlać tylko blogi dla osób współdzielących grupę';
$string['intro'] = 'Ten strumień RSS został wygenerowany automatycznie z jednego lub wielu blogów';
$string['noentriesyet'] = 'Brak widzialnych wpisów w tym miejscu';
$string['noguestpost'] = 'Goście nie mogą wysyłać blogów!';
$string['norighttodeletetag'] = 'Nie masz praw do usuniecia tego znacznika - $a';
$string['notallowedtoedit'] = 'Nie możesz edytować tego wpisu';
$string['numberofentries'] = 'Wpisy: $a';
$string['numberoftags'] = 'Liczba znaczników do wyświetlenia';
$string['otags'] = 'Oficialne znaczniki';
$string['pagesize'] = 'Liczba wpisów blogu na Stronę';
$string['personalblogs'] = 'Użytkownicy mogą wyświetlać tylko własny blog';
$string['ptags'] = 'Znaczniki definiowane przez użytkownika';
$string['publishto'] = 'Opublikuj do';
$string['publishtonoone'] = 'Siebie (draft)';
$string['publishtosite'] = 'Wszystkich na tej witrynie';
$string['publishtoworld'] = 'Wszystkich na świecie';
$string['settingsupdatederror'] = 'Wystąpił błąd, ustawienia preferencji bloga nie mogły zostać uaktualnione';
$string['siteblog'] = 'Blog witryny: $a';
$string['siteblogs'] = 'Wszyscy użytkownicy witryny mogą wyświetlać wszystkie wpisy blogu';
$string['tagdatelastused'] = 'Znacznik daty był ostatnio użyty';
$string['tagsort'] = 'Sortuj listę znaczników według';
$string['tagtext'] = 'Tekst znacznika';
$string['timewithin'] = 'Wyświetl znaczniki używane w ciągu tej liczby dni';
$string['updateentrywithid'] = 'Uaktualnianie wpisu';
$string['userblog'] = 'Blog użytkownika: $a';
$string['viewcourseentries'] = 'Wyświetl wpisy kursu';
$string['viewmyentries'] = 'Wyświetl moje wpisy';
$string['viewsiteentries'] = 'Wyświetl wpisy witryny';
$string['worldblogs'] = 'Wszyscy mogą czytać wpisy ustawione jako dostępne dla świata';

?>
