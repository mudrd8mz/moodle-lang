...
$string['auth_pamdescription'] = 'この方法では、このサーバのネイティブユーザ名にアクセスする手段として、PAMを使用します。このモジュールを使用するために、あなたは、<a href=\"http://www.math.ohio-state.edu/~ccunning/pam_auth/\">PHP4 PAM Authentication</a>をインストールする必要があります。';
...
$string['auth_shib_idp_list_description'] = 'ログインページでユーザが選択できるよう、アイデンティティプロバイダのエンティティIDリストを入力してください。<br />それぞれの行には、アイデンティティプロバイダのエンティティIDに関するカンマ区切りのタプル (tuple) を入力してください (詳細は、Shibbolethメタデータファイルをご覧ください)。アイデンティティプロバイダ名がドロップダウンリストに表示されます。<br />あなたのMoodleが複数連盟設定の一部にある場合、任意の第3パラメータとして、Shibbolethセッションイニシエータのロケーションを追加することができます。';
...
$string['auth_shib_instructions_help'] = 'Shibbolethに関して、ユーザに提示する説明文です。 これは、ログインページの説明セクションに表示されます。Shibbolethユーザが簡単にログインできるよう、「<b>$a</b>」 のようなリンクを含んでください。';
...
$string['auth_updateremote_ldap'] = '<p><b>注意:</b> 外部LDAPデータを更新するには、すべてのユーザレコードに書き込み権を持ったバインドユーザに対して、binddnおよびbindpwを設定する必要があります。現在、多値属性を保持することはできません。また、超過した値は、更新時に取り除かれます。</p>';
...
$string['forgottenpassword'] = 'あなたがここにURIを入力した場合、このサイトの喪失パスワードリカバリページに使用されます。これは、完全にMoodleの外でパスワードが処理されるサイトを対象としています。デフォルトのパスワードリカバリを使用する場合、空白のままにしてください。';
...
$string['errormaxconsecutiveidentchars'] = 'パスワードには、最大 $a 文字の連続した同一文字を使用する必要があります。'; // ORPHANED
$string['sso_idp_description'] = 'このサービスを公開することで、あなたのユーザは、再度ログインせずに $a のMoodleサイトを散策することができます。<ul><li><em>従属関係</em>: あなたは $a のSSO (サービスプロバイダ) サービスに<strong>登録</strong>する必要があります。</li></ul><br />このサービスに登録することで、$a からの認証済みユーザは、再度ログインせずにあなたのサイトにアクセスできます。<ul><li><em>従属関係</em>: あなたは $a にSSO (サービスプロバイダ) サービスを<strong>公開</strong>する必要があります。</li></ul><br />'; // ORPHANED
...
$string['user_activatenotsupportusertype'] = 'auth: ldap user_activate() は、選択したユーザタイプをサポートしていません: $a'; // ORPHANED
$string['user_disablenotsupportusertype'] = 'auth: ldap user_disable() は、選択したユーザタイプをまだサポートしていません。'; // ORPHANED
