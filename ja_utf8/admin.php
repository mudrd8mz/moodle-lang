...
$string['configallowuserswitchrolestheycantassign'] = 'デフォルトでは、moodle/role:assignは、ロールをスイッチするユーザに必要です。この設定を有効にすることで、この必要条件を解除します。また、「ロールを切り替える ...」ドロップダウンメニューで利用できるロールは、「ロールの割り当て許可」テーブルの設定のみで決定されることになります。「ロールの割り当て許可」テーブルの設定では、ユーザの既存ロールより多くのケイパビリティを持ったロールへの切り替えを許可しないことをお勧めします。';
$string['configallowuserthemes'] = 'この設定を有効にした場合、ユーザは自分のテーマを設定することができます。サイトテーマは、ユーザテーマに上書きされます (コーステーマは上書きされません)。';
$string['configallusersaresitestudents'] = 'サイトのフロントページにある活動に関して、すべてのユーザを学生と考えてもよろしいですか? この設定を「Yes」にした場合、承認済みユーザアカウントのユーザは、学生として、これらの活動に参加することができます。この設定を「No」にした場合、少なくとも1つのコースに登録しているユーザのみ、フロントページの活動に参加することができます。管理者および特別に割り当てられた教師のみ、これらのフロントページの活動に関して、教師として行動することができます。';
...
$string['configbloglevel'] = 'ここでは、どのユーザブログをこのサイトで閲覧できるか、アクセスレベルを制限することができます。ここで設定するのは、閲覧者に関する最大のコンテクストであり、投稿者またはブログ投稿のタイプではないことに注意してください。不要な場合、ブログを完全に無効にすることもできます。';
...
$string['configclamfailureonupload'] = 'アップロードファイルをclamがスキャンするよう設定したとして、間違って設定されている、または不明な理由で動作しない場合、あなたは、どのように処理しますか? 「ファイルをウイルスとして扱う」を選択した場合、感染ファイルは隔離用エリアに移動されるか、削除されます。「ファイルをウイルス感染なしとして扱う」を選択した場合、通常のファイルと同じように、指定したディレクトリに移動されます。どちらの場合でも、管理者にはclamに問題がある旨が通知されます。「ファイルをウイルスとして扱う」を選択してもclamが正常に動作しない場合 (一般的にclamに間違ったパスを指定)、アップロードされたすべてのファイルは、指定された隔離用エリアに移動されるか、削除されます。この設定には、注意してください。';
$string['configcookiehttponly'] = 'ブラウザが実HTTPリクエストのみ送信する、PHP 5.2.0の新しい機能を有効にすることで、スクリプト言語からクッキーにアクセスできないようになります。この機能は、すべてのブラウザでサポートされているわけではありません。また、現在のコードと完全な互換性はありません。いくつかのタイプのXSSアタックを防ぐためには役立ちます。';
...
$string['configcoursemanager'] = 'ここでは、誰がコース説明に表示されるか設定することができます。ユーザがコース説明に表示されるためには、これらの中から少なくとも１つのコースロールを割り当ててください。';
...
$string['configcreatornewroleid'] = 'このロールは、新しいコースを作成したコース作成者に自動的に割り当てられます。コース作成者が、親コンテクストで必要なケイパビリティを持っている場合、このロールは割り当てられません。';
...
$string['configenableajax'] = 'ここでは、サイト全体におけるAJAX (Javaスクリプトを使用した高度なクライアント/サーバインターフェース) の使用をコントロールできます。この設定を有効にした場合、ユーザはプロファイルページでAJAXの使用を選択できます。この設定を無効にした場合、すべてのユーザに関して、AJAXの使用が無効にされます。';
$string['configenablecalendarexport'] = 'カレンダーのエクスポートまたは購読を有効にします。';
$string['configenablecourserequests'] = 'すべてのユーザにコース作成のリクエストを許可します。';
$string['configenableglobalsearch'] = 'ここでは、リソースおよび活動のグローバルテキストサーチを有効にします。この設定は、PHP4での互換性はありません。';
$string['configenablegroupings'] = 'ここでは、グループのグルーピングを有効にします。';
$string['configenablehtmlpurifier'] = '信頼できないテキストのクリーニングに関して、KSESの代わりにHTML Purifierを使用します。HTML Purifierは活発に開発され、さらにセキュアだと考えられていますが、リソースに負荷がかかります。HTML Purifierでは、表示されるHTMLコードに若干の違いが生じると予想されます。embedおよobjectタグを有効にできないこと、MathMLタグおよび古いlangタグがサポートされないことに留意してください。';
...
$string['configenrolmentplugins'] = 'あなたが使用したいプラグインを選択してください。また、忘れずに適切に設定してください。<br /><br />ここでは、どのプラグインを有効にするのか指定する必要があります。<strong>1つ</strong>のプラグインをインタラクティブなユーザ登録方法のデフォルトとして設定することができます。インタラクティブなユーザ登録方法を無効にするには、該当するコースの設定画面で「コース登録可能」を「No」にしてください。';
$string['configerrorlevel'] = '表示したいPHPの警告数を選択してください。通常は、Normalが最適な選択です。';
$string['configexcludeoldflashclients'] = 'いくつかのAdobe Flashでは、悪意を持ったFlashコンテンツにより、脆弱性を突いた攻撃を受ける可能性があることを知られています。あなたはここで、サポートするバージョンの最低限度を指定することができます。ここで指定したバージョンより古いバージョンの場合、MoodleはFlashファイルを表示しません。代わりに、アップグレード方法に関する代替Flashファイルが表示されます。すべてのチェックを無効にするには、ここを空のままにしてください。';
...
$string['configforceloginforprofiles'] = 'この設定では、ユーザプロファイルページの閲覧に関して、(ゲストではない) 実アカウントでのログインをユーザに強制します。この設定を無効にした場合、ユーザが広告 (スパム) を投稿、または他の不適切なコンテンツをプロファイルに投稿して、世界中から閲覧できる可能性があります。';
...
$string['configfrontpageloggedin'] = '上で選択したアイテムは、ユーザのログイン時、サイトフロントページに表示されます。';
$string['configfullnamedisplay'] = 'ここでは、氏名がどのように表示されるか設定します。単一言語を使用したほとんどのサイトでは「名 + 姓」を使用した方が良いでしょう。しかし、あなたは、姓を隠したり、現在の言語パックが指定したままにすることも可能です (いくつかの言語では、異なる並びとなります)。';
$string['configgdversion'] = 'インストールされているGDのバージョンを選択してください。デフォルト値として表示されているバージョンは、自動判別されたものです。あなたが何をしているのか分からない場合、そのままにしてください。';
$string['configgeoipfile'] = 'GeoIP Cityバイナリデータファイルのロケーションです。このファイルは、Moodleディストリビューションの一部ではなく、<a href=\"http://www.maxmind.com/\">MaxMind</a>から個別に取得する必要があります。あなたは、商用バージョンを購入、または無料バージョンを使用することができます。<br />無料バージョンの場合、<a href=\"http://www.maxmind.com/download/geoip/database/GeoLiteCity.dat.gz\">http://www.maxmind.com/download/geoip/database/GeoLiteCity.dat.gz</a>からダウンロードして、あなたのサーバの「 $a 」ディレクトリに解凍してください。';
...
$string['configguestroleid'] = 'このロールは、自動的にゲストユーザに割り当てられます。また、パスワードなしで受講できるコースに入ったユーザに対しても、一時的に割り当てられます。このロールが「moodle/legacy:guest」および「moodle/course:view」ケイパビリティを持っているか確認してください。';
...
$string['confightmleditor'] = 'Moodleに内蔵されたHTMLテキストエディタの使用を許可するかどうか選択してください。あなたが有効に選択した場合でも、互換性のあるブラウザ使用時のみ、このエディタが表示されます。ユーザは、HTMLエディタを使用しない選択をすることもできます。';
$string['configidnumber'] = 'このオプションでは、次の内容を設定します。(a)ユーザがユーザIDの入力を求められない、(b)ユーザがユーザIDを空白にすることができる、(c)ユーザはユーザIDの入力を求められ、空白にできない。ユーザIDが入力された場合、ユーザのプロファイルに表示されます。';
$string['configintcachemax'] = 'この値は、内部キャッシュのみに使用します。キャッシュに保持する最大レコード数です。推奨値は50です。 メモリ使用量を減らすには、値を低くしてください。';
$string['configintro'] = 'このページでは、あなたのサーバでMoodleが最適に動くための設定値を入力できます。設定値を過度に心配しないでください - 通常、デフォルト値で最適に動作するよう、設定されています。いつでもこのページに戻って再度設定することができます。';
$string['configintroadmin'] = 'このページでは、サイトすべてを管理できる最高管理者のアカウントを作成します。正しいメールアドレスおよび安全なユーザ名、パスワードを設定してください。あなたは、後でさらに管理者アカウントを作成することができます。';
$string['configintrosite'] = 'このページでは、このサイトのフロントページおよびサイト名を設定します。管理メニューを使用することで、いつでもここに戻って、これらの設定を変更することができます。';
$string['configintrotimezones'] = 'ここでは、世界のタイムゾーン情報 (夏時間ルールを含む) を検索して、あなたのローカルデータベースを更新します。次のロケーションが順番にチェックされます: $a この処理は一般的に非常に安全であり、標準インストールのMoodleを壊すことはありません。タイムゾーン情報を更新してもよろしいですか?';
$string['configiplookup'] = 'あなたがログ等でIPアドレス (例 34.12.222.93) をクリックした場合、そのIPアドレスがどこから来たものなのか推量しやすいマップが表示されます。この機能には、あなたが選択できる異なるプラグインがあり、それぞれに利点と不利点があります。';
$string['configkeeptagnamecase'] = 'ユーザが入力したタグ名の大文字小文字を保持したい場合、この設定を有効にしてください。';
$string['configlang'] = 'サイト全体に適用されるデフォルト言語を選択してください。ユーザは、この設定を後で変更することができます。あなたがデータベース接続問題に関するローカライズされたメッセージを表示させたい場合、config.phpに \$CFG->lang=\"yourlangcode_utf8\"; と設定することもできます。';
...
$string['configlangmenu'] = '汎用の言語選択メニューをホームページ、ログイン画面等に表示するかどうか選択してください。この設定は、ユーザプロファイルでユーザが表示言語を選択することに影響を及ぼしません。';
$string['configlatinexcelexport'] = 'Excelエクスポートのエンコーディングを選択してください。';
$string['configlocale'] = 'サイト全体に適用されるロケールを選択してください - この設定は、すべての言語パックの日付の書式および言語を上書きします (カレンダーの曜日には影響しません)。あなたのオペレーティングシステムにロケールデータがインストールされている必要があります (例 linuxの場合、ja_JP.UTF-8、en_US.UTF-8、es_ES.UTF-8)。 ほとんどの場合、このフィールドは、空白のままにしてください。';
$string['configloginhttps'] = 'この設定を有効にした場合、Moodleはログインページのみセキュアhttps接続 (セキュアログイン) を使用します。その後、一般的な速度を保つため、通常のhttp URIに戻ります。警告: この設定では、ウェブサーバのhttpsを厳密に有効にする必要があります。もし、httpsが有効ではない状態でこの設定を有効にした場合、あなたは自分のサイトから閉め出されることになります。';
$string['configloglifetime'] = 'ここでは、ユーザの活動に関するログの保持期間を設定します。指定した値より古いログは、自動的に削除されます。あなたが必要な場合、可能な限り長い間ログを保持する方が良いのですが、アクセス件数が多く、サーバのパフォーマンスに問題がある場合、ログの保持期間を短い期間に設定してください。統計が正常に動作しないため、30以下の値はお勧めできません。';
$string['configlongtimenosee'] = '学生が長期間ログインしていない場合、自動的にコースから登録抹消されます。この値では、登録抹消期限を決定します。';
$string['configlookahead'] = '直近イベント日';
$string['configmailnewline'] = 'メールメッセージに使用される改行文字です。RFC 822bisによるとCRLFは必須です。いくつかのメールサーバでは、自動的にLFがCRLFに変換され、他のメールサーバではCRLFがCRCRLFに誤って変換され、また、生のLFを拒否するメールサーバ (例えば、qmail) もあります。メールが送信されなかったり、改行が2重になる問題がある場合、この設定を変更してください。';
$string['configmaxbytes'] = 'ここでは、サイト全体でアップロードできるファイルの最大サイズを設定します。この設定は、PHP設定のpost_max_sizeおよびupload_max_filesize、同様にApache設定のLimitRequestBodyに制限されます。maxbytesは、コースレベルまたはモジュールレベルで選択できるサイズの範囲を制限します。「サーバリミット」が選択された場合、サーバで設定された最大値が使用されます。';
$string['configmaxeditingtime'] = 'ここでは、フォーラム投稿、用語集コメント等を編集可能な時間を設定します。通常30分が適切な値です。';
...
$string['configmymoodleredirect'] = 'この設定では、管理者以外のユーザがログインした場合、強制的に「/my」にリダイレクトします。また、トップレベルのサイトナビゲーションを「/my」に置き換えます。';
$string['confignodefaultuserrolelists'] = 'デフォルトロールがサイトコースにアクセスを提供している場合、この設定では、廃止予定のget_course_user等のコールを使用してデータベースからすべてのユーザをデフォルトロールに戻さないようにします。パフォーマンスに問題がある場合、この設定を有効にしてください。';
...
$string['configopentogoogle'] = 'この設定を有効にした場合、Googleはあなたのサイトにゲストとして入ることができます。さらに、Googleが自動的に記録したデータを元に、他の人がゲストとしてあなたのサイトにアクセスできます。これは、すでにゲストアクセスを許可しているコースのみ、透過アクセスを提供することに留意してください。';
...
$string['configregenloginsession'] = 'それぞれのログインリクエストに関して、セッションIDの再作成を強くお勧めします。この設定は、いくつかの認証プラグインとの互換性がありません。';
...
$string['configrestrictmodulesfor'] = 'どのコースで、いくつかの活動モジュールを無効にするよう<b>設定</b>しますか? この設定は、教師にのみ適用されることに留意してください。管理者は、どのような活動でもコースに追加することができます。';
$string['configrunclamavonupload'] = 'この設定を有効にした場合、すべてのアップロードファイルをスキャンするため、clam AVが使用されます。';
$string['configrunclamonupload'] = 'ファイルアップロード時にclam AVを動作させますか? 正常に動作させるには、pathtoclamに正しいパスを入力する必要があります (Clam AVは次の場所で入手できる無料ウイルススキャナです: http://www.clamav.net/)。';
...
$string['configsecureforms'] = 'フォームデータを受入れる場合、Moodleでは、追加的なレベルのセキュリティを使用することができます。この設定を有効にした場合、ブラウザのHTTP_REFERER変数とフォーム設置のアドレスが照合されます。 ユーザがファイアーウォールソフト (例 Zonealarm) を使用してトラフィックからHTTP_REFERERを取除く設定をしている場合、まれに問題が生じる場合があります。 これは、フォーム使用時に画面が「固まる」現象です。例えば、ユーザがログインページで問題を抱えている場合、この設定をNoにしてください。しかし、この設定では、あなたのサイトがブルートフォース・パスワードアタックに対して、無防備な状態になります。分からない場合、この設定を「Yes」のままにしてください。';
...
$string['configsessioncookiedomain'] = 'ここでは、Moodleクッキーが利用できるドメインを変更します。これは、別のサブドメインのウェブアプリケーションとMoodleセッションを共有する必要のある、特別なMoodle (例 認証または登録プラグイン) に有用です。<strong>警告: この設定をデフォルト (空) のままにすることを強くお勧めします - 正しくない値を設定することで、すべてのユーザがサイトにログインできなくなります。</strong>';
...
$string['configsitemaxcategorydepthhelp'] = 'ここでは、表示される子カテゴリの最大深度を設定します。';
...
$string['configstatscatdepth'] = '統計コードでは、簡易化したコース受講登録ロジックを使用し、オーバーライドは無視されます。また、ここでは、検証された親コースカテゴリ数を設定します。設定値ゼロの場合、サイトおよびコースレベルの直接ロール割り当てのみ検出します。設定値1の場合、コースの親カテゴリ等のロール割り当ても同時に検出します。設定値を大きくすることで、統計処理中、さらにデータベースサーバに過大な負荷がかかることになります。';
$string['configstatsfirstrun'] = 'ここでは、cronジョブが<b>最初の</b>統計処理で、どのくらい過去にさかのぼるか指定します。トラフィックが多く、共有ホスティングを利用している場合、あまりにも過去にさかのぼること、良い考えではありません。統計処理は長時間を必要とし、リソースに過大な負荷がかかります (この設定では、1ヵ月 = 28日ですので注意してください。グラフおよびレポートの生成には、1ヶ月 = 1カレンダー月が利用されます)。';
$string['configstatsmaxruntime'] = '統計処理では、サーバに大きな負荷がかかる場合があります。このオプションおよび次のオプションの組み合わせで、「いつ」および「どのくらい」稼動させるか設定してください。';
$string['configstatsmaxruntime2'] = '統計処理では、サーバに大きな負荷がかかる場合があります。1日の統計データの収集で許可する最大時間を指定してください。1回のcron実行で処理できる最大日数は、31です。';
...
$string['configteacherassignteachers'] = '一般の教師は、自分が教えるコースに他の教師を割り当てることができますか? 「No」の場合、コース作成者および管理者のみ、教師を割り当てることができます。';
...
$string['configtimezone'] = 'ここでは、デフォルトタイムゾーンを設定することができます。これは日付表示のためのデフォルトタイムゾーンを設定するのみで、各ユーザはプロファイル設定画面にて、ここで設定されたタイムゾーンを変更することができます。「サーバのシステム時間」を選択することにより、オペレーティングシステムの時間設定をMoodleのデフォルトにします。これに対して、ユーザプロファイル設定画面のタイムゾーンで選択する「サーバのシステム時間」では、ここで設定したタイムゾーンをユーザデフォルトにします。時刻に依存するcronジョブは、このタイムゾーンを使用して実行されます。';
...
$string['confirminstall'] = 'あなたは、言語パック ($a) をインストールしようとしています。本当によろしいですか?';
...
$string['displayerrorswarning'] = 'いくつかのエラーメッセージでは、あなたのサーバに関する機密情報を公開する場合があります。実稼動サイトでのPHP設定「display_errors」の有効化はお勧めできません。';
$string['displayloginfailures'] = 'ログイン失敗を表示する';
$string['docroot'] = 'Moodle Docsドキュメントルート';
$string['doctonewwindow'] = '新しいウィンドウに開く';
$string['download'] = 'ダウンロード';
$string['editennotallowed'] = 'このページでは、言語ファイル「en_utf8」を編集することはできません。ローカルに切り替えてください。';
$string['edithelpdocs'] = 'ヘルプドキュメントの編集';
$string['editingnoncorelangfile'] = 'あなたは、アドオンモジュール/プラグインの翻訳済み言語ファイルを修正しようとしています。あなたは、サードパーティの翻訳言語ファイルを_localフォルダのみに保存することができます。サードパーティの翻訳済み言語ファイルは、モジュールの言語ディレクトリに移動するか、アドオンモジュールの管理者に送信してください。';
...
$string['editorspellinghelp'] = 'ここでは、スペルチェックをするかどうか設定します。有効にした場合、サーバに<strong>aspell</strong> をインストールする必要があります。';
...
$string['helpadminseesall'] = '管理者は、カレンダーイベントすべてを閲覧または自分自身のイベントとすることができますか?';
...
$string['invaliduserchangeme'] = 'ユーザ名「changeme」は、予約されています -- あなたは、このユーザ名のアカウントを作成することができません。';
...
$string['profilecategorynamenotunique'] = 'このカテゴリ名は、すでに使用されています。';
...
$string['profileshortnamenotunique'] = 'この省略名は、すでに使用されています。';
...
$string['questioncwqpfsok'] = '素晴らしい。あなたの小テストでは、共有および非共有カテゴリから混在して選択されるよう、設定されている「ランダム」問題はありません。';
...
$string['cannotdeleteqtypeinuse'] = 'あなたは、問題タイプ「 $a 」を削除することはできません。このタイプの問題が問題バンクに登録されています。'; // ORPHANED
$string['cannotdeleteqtypeneeded'] = 'あなたは、問題タイプ「 $a 」を削除することはできません。この問題タイプに依存する他の問題タイプがインストールされています。'; // ORPHANED
...
$string['clistatusdisabled'] = 'ステータスは、無効にされています。'; // ORPHANED
$string['clistatusenabled'] = 'ステータスは、有効にされています。'; // ORPHANED
...
$string['condifmodeditdefaults'] = 'ここでは、あなたが新しい活動を作成する場合に使用される、活動設定フォームのデフォルト値を定義します。また、どの活動設定が「高度な設定」なのか設定することもできます。'; // ORPHANED
...
$string['configdebugstringids'] = 'このオプションは、翻訳者を手助けするよう、作成されました。それぞれ表示されるストリングの横に言語ファイルおよびストリングIDを表示します (この設定を変更した場合、次のページ読み込みからのみ反映されます)。'; // ORPHANED
$string['configdebugvalidators'] = 'この設定を有効にした場合、フッタに外部Validatorサーバのリンクが表示されます。あなたは、新たなユーザ名「w3cvalidator」を作成して、ゲストアクセスを有効にする必要があります。これらの変更により、サーバに対して、不正なアクセスを許可してしまうことになります。実稼動サイトでは、有効にしないでください!'; // ORPHANED
$string['configenablecomments'] = 'コメントを有効にします。'; // ORPHANED
$string['configenablewebservices'] = 'ウェブサービスでは、他のシステムがこのMoodleにログインして、操作できるようにします。セキュリティのため、あなたが本当に使用したい場合以外、この機能を無効にしてください。'; // ORPHANED
...
$string['configmaxconsecutiveidentchars'] = 'パスワードには、この値以上の連続した同一文字を使用することができません。このチェックを無効にするには、ゼロを指定してください。'; // ORPHANED
...
$string['deletefilterareyousuremessage'] = 'あなたは、フィルタ「 $a 」を削除しようとしています。本当にアンインストールしてもよろしいですか?'; // ORPHANED
...
$string['deleteqtypeareyousuremessage'] = 'あなたは、問題タイプ「 $a 」を完全に削除しようとしています。本当にアンインストールしてもよろしいですか?'; // ORPHANED
...
$string['navigationupgrade'] = 'このアップグレードでは、次のブロックに替えて、新しい2つのナビゲーションブロックが導入されます: 管理、コース、活動および参加者。これらのブロックに特別なパーミッションを設定している場合、あなたの希望するとおりに、すべてが動作するかどうか確認してください。<br /><br />また、新しいスタイルを読み込むため、あなたのブラウザを「最新の情報に更新」してください。そうでない場合、新しいブロックは正常に動作しません。'; // ORPHANED
...
$string['settingfileuploads'] = 'ファイルアップロードは、通常の操作に必要です。PHP設定で有効にしてください。'; // ORPHANED
...
$string['sitemaintenancewarning2'] = 'あなたのサイトは、現在メンテナンスモードで運用されています (管理者のみログインできます)。通常運用に戻すには、<a href=\"$a\">メンテナンスモードを解除してください</a>。'; // ORPHANED
