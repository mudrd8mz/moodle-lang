<?PHP // $Id$ 
      // admin.php - created with Moodle 2.0 dev (Build: 20091021) (2009102100)
...
$string['configenablestats'] = 'この設定を有効にした場合、Moodleのcronジョブは、ログを処理して統計情報を集めます。あなたのサイトのトラフィックにより、しばらく時間がかかる場合があります。このオプションを有効にした場合、あなたの各コースまたはサイト全体に関して、面白いグラフおよび統計を閲覧することができます。';
...
$string['configenrolmentplugins'] = 'あなたが使用したいプラグインを選択してください。また、忘れずに適切に設定してください。<br /><br />ここでは、どのプラグインを有効にするのか指定する必要があります。<strong>1つ</strong>のプラグインを<em>インタラクティブ</em>なユーザ登録方法のデフォルトとして設定することができます。インタラクティブなユーザ登録方法を無効にするには、該当するコースの設定画面で「コース登録可能」を「No」にしてください。';
...
$string['configforceloginforprofiles'] = 'この設定では、ユーザプロファイルページの閲覧に関して、(ゲストではない) 実アカウントでのログインをユーザに強制します。この設定を無効にした場合、ユーザが広告 (スパム) を投稿、または他の不適切なコンテンツをプロファイルに投稿して、世界全体から閲覧できる可能性があります。';
...
$string['configgooglemapkey'] = 'IPアドレスのルックアップを視覚化するため利用するGoogle Mapsのスペシャルキーを入力してください。あなたは、<a href=\"http://code.google.com/apis/maps/signup.html\">http://code.google.com/apis/maps/signup.html</a>より、キーを無償で取得することができます。<br />あなたのウェブサイトURIは: $a';
...
$string['confighiddenuserfields'] = 'あなたが、コース教師/管理者以外のユーザに隠したいユーザ情報フィールドを選択してください。この設定により、ユーザのプライバシーを強化することができます。複数のフィールドを選択するには、CTRLキーを押したままにしてください。';
...
$string['configintrosite'] = 'このページでは、このサイトのフロントページおよびサイト名を設定します。管理メニューの「サイト設定」リンクを使用することで、いつでもここに戻って、これらの設定を変更することができます。';
...
$string['configlangcache'] = '言語メニューをキャッシュします。多くのメモリーおよびCPU処理能力を節約することができます。この設定を有効にした場合、言語パッケージの追加または削除に数分間の更新時間を要します。';
...
$string['configlocale'] = 'サイト全体に適用されるロケールを選択してください - この設定は、すべての言語パックの日付の書式および言語を上書きします (カレンダーの曜日には影響しません)。オペレーティングシステムにロケールデータがインストールされている必要があります (例 linuxの場合、ja_JP.UTF-8、en_US.UTF-8、es_ES.UTF-8)。 ほとんどの場合、このフィールドは、空白のままにしてください。';
...
$string['configmemcachedhosts'] = 'memcached用の設定です。memcachedデーモンを動作させるホスト一覧をカンマで区切って入力してください。DNSの遅延を避けるには、IPアドレスを使用してください。動作中のMoodleからホストを追加/削除した場合、memcachedが正常に機能しない可能性があります。';
...
$string['configsecureforms'] = 'フォームデータを受入れる場合、Moodleでは、追加的なレベルのセキュリティーを使用することができます。この設定を有効にした場合、ブラウザのHTTP_REFERER変数とフォーム設置のアドレスが照合されます。 ユーザがファイアーウォールソフト (例 Zonealarm) を使用してトラフィックからHTTP_REFERERを取除く設定をしている場合、まれに問題が生じる場合があります。 これは、フォーム使用時に画面が「固まる」現象です。例えば、ユーザがログインページで問題を抱えている場合、この設定をNoにしてください。しかし、この設定ではブルートフォース・パスワードアタックに対して、無防備な状態になります。分からない場合は、この設定を「Yes」のままにしてください。';
...
$string['configslasharguments'] = 'ファイル (イメージ、アップロード等) は、スラッシュを使ってプログラムに受け渡されます。この方法により、簡単にブラウザ内やプロクシにイメージを一時保存させることができます。いくつかのPHPサーバでは、この方法が取れない場合があります。もし、アップロードされたファイルまたはイメージ (例 ユーザ画像) の表示に問題がある場合、この設定を無効にしてください。';
...
$string['configsmtpuser'] = '上でSMTPサーバを設定し、なおかつ認証が必要な場合、ユーザ名およびパスワードを設定してください。';
...
$string['configstatsfirstrun'] = 'ここでは、cronジョブが<b>最初の</b>統計処理で、どのくらい過去にさかのぼるか指定します。トラフィックが多く、共有ホスティングを利用している場合、あまりにも過去にさかのぼるのは良い考えではありません。統計処理は長時間を必要とし、リソースに過大な負荷がかかります (この設定では、 1ヵ月 = 28日ですので注意してください。グラフおよびレポートの生成には、1ヶ月= 1カレンダー月が利用されます)。';
...
$string['configstripalltitletags'] = 'この設定を無効にした場合、活動およびリソース名において、HTMLタグ使用を許可します。';
...
$string['configverifychangedemail'] = '変更済みメールアドレスの確認に関して、「許可されるメールアドレスのドメイン」および「拒否されるメールアドレスのドメイン」設定を使用します。この設定が無効にされた場合、新しいユーザ作成時のみドメインが強制されます。';
...
$string['settingsafemode'] = 'Moodleではセーフモードに完全な互換性がありません。サーバ管理者にセーフモードの無効化を依頼してください。セーフモードでのMoodleの動作はサポートされていません。セーフモードでMoodleを動作させる場合、様々な問題が発生する可能性があります。';
