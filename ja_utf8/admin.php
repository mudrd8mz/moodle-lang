...
$string['configcachetype'] = 'Moodleが使用するキャッシュのタイプを選択してください。ここではキャッシュのみ設定します。キャッシュが使用されるためには、忘れずにレコードキャッシュ (rcache) 設定を有効にしてください。あなたがデータベースシステムのロードを減らしたい場合<strong>のみ</strong>使用してください -- そうでない場合、Moodleの動作が実際に遅くなります。中トラフックのサイトでは「internal」の選択でメリットがあります。eAcceleratorまたはTurckmmcacheがインストールされ、<em>shared memoryオプションが有効にされた</em>単体のウェブサーバでは、「eaccelerator」を選択してください。複数のサーバを使用し、1つまたはそれ以上のmemcachedデーモンが動作およびPHP-memcachedエクステンションが設定されている場合、「memcached」を選択して、下記のmemachedオプションを設定してください。<br /><strong>注意:</strong> ワークロード下でパフォーマンスを確認し、それに応じて調整してください -- キャッシュは、あなたのサイトを遅くする場合があります。高トラフィックの場合、eAcceleratorおよびmemcachedの使用に大きなメリットがありますが、ウェブサーバのCPU使用に関して、高いコストがかかります。';
...
$string['configintcachemax'] = 'この値は内部キャッシュのみに使用します。キャッシュに保持する最大レコード数です。推奨値は50です。 低い値を使用することでメモリ使用量を減らします。';
...
$string['profilecategoryname'] = 'カテゴリ名 (一意にしてください)';
...
$string['profilecreatefield'] = '新しいプロフィールフィールドの作成:';
$string['profilecreatenewcategory'] = '新しいカテゴリの作成';
$string['profilecreatenewfield'] = '新しいプロフィールフィールド「 $a 」の作成';
$string['profiledefaultcategory'] = '他のフィールド';
$string['profiledefaultdata'] = '初期値';
