...
$string['choosefiletoedit'] = '上記より編集するファイルを選択してください。';
...
$string['configallowoverride'] = '左側のロールに各カラムのロールのオーバライドを許可します。';
...
$string['configauthenticationplugins'] = 'あなたが使用したい認証プラグインを選択して、フェイルスルーの順に並び替えてください。自己登録は、「認証」カラムで選択されたプラグインに使用されます (通常、Emailベースの認証)。';
...
$string['configclamfailureonupload'] = 'アップロードファイルをclamがスキャンするよう設定しても、設定が間違っている、または不明な理由で動作しない場合、どのように処理しますか? 「ファイルをウイルスとして扱う」を選択した場合、感染ファイルは隔離用エリアに移動されるか、削除されます。「ファイルをウイルス感染なしとして扱う」を選択した場合、通常のファイルと同じように、指定したディレクトリに移動されます。どちらの場合でも、管理者にはclamに問題がある旨が通知されます。「ファイルをウイルスとして扱う」を選択して、clamが正常に動作しない場合 (一般的にclamに間違ったパスを指定)、アップロードされたすべてのファイルは、指定された隔離用エリアに移動されるか、削除されます。この設定は注意して行ってください。';
$string['configcountry'] = 'ここで国を設定した場合、新規アカウントのユーザにデフォルトとして選択されます。ユーザに国を選ばせる場合、設定しないままにしてください。';
$string['configcoursemanager'] = 'この設定では、誰がコース説明に表示されるかコントロールします。ユーザがコース説明に表示されるには、コース内で上記のロールが少なくとも1つ割り当てられている必要があります。';
$string['configcoursesperpage'] = 'コース一覧の1ページあたりに表示するコース数を入力してください。';
$string['configcronclionly'] = 'この設定を有効にした場合、cronスクリプトはウェブ経由の代わりに、コマンドラインのみで実行することができます。この設定は、下記のcronパスワード設定をオーバーライドします。';
$string['configcronremotepassword'] = 'ここでは、下記の形式のURLを使用してパスワードを提供しない限り、cron.phpが動作しないことを意味します。<pre>
...
$string['configenablerecordcache'] = 'この設定を0以上に設定した場合、データベースから情報を取得する際にインメモリキャッシングが実行されます。このことで、データベースパフォーマンスは改善されますが、一方ではサーバのメモリ要件が若干増加します。あなたのサーバに搭載されているメモリが少なくない場合、一般的にこの設定を50前後にすることをお勧めします。開発者がパフォーマンス向上を確信する場合、いくつかのプロセスでは、この設定がオーバライドされます。';
...
$string['configloginhttps'] = 'この設定を有効にした場合、Moodleはログインページのみでセキュアhttps接続 (セキュアログイン) を使用します。その後、一般的な速度を保つため、通常のhttp URLに戻ります。警告: この設定では、ウェブサーバのhttpsを明確に有効にする必要があります。もし、httpsが有効ではない状態でこの設定を有効にした場合、あなたは自分のサイトから閉め出されることになります。';
$string['configloglifetime'] = 'ここでは、ユーザの活動に関するログの保持期間を設定します。指定した値より古いログは自動的に削除されます。可能な限り長い間ログを保持する方が良いのですが、アクセス件数が多く、サーバのパフォーマンスに問題がある場合、短い期間に設定してください。';
...
$string['configproxyhost'] = '<b>サーバ</b>のインターネットアクセスにプロクシ (例 ファイアーウォール) が必要な場合、プロクシのホスト名を入力してください。必要でない場合は空白にしてください。';
$string['configproxyport'] = 'サーバがプロクシを必要とする場合、ここにプロクシのポート番号を入力してください。';
$string['configquarantinedir'] = 'clam AVに感染ファイルを隔離用ディレクトリに移動させたい場合、ここにそのディレクトリを入力してください。このディレクトリは、ウェブサーバより書き込みができるようにする必要があります。ここを空白にした場合、または存在しないか、書き込み権が無いディレクトリを指定した場合、感染ファイルは削除されます。末尾にスラッシュを付けないでください。';
...
$string['configsecureforms'] = 'Moodleでは、フォームデータを受入れる場合の追加的なレベルのセキュリティーを使用することができます。この設定を有効にした場合、ブラウザのHTTP_REFERER変数とフォーム設置のアドレスが照合されます。 ユーザがファイアーウォールソフト (例 Zonealarm) を使用してトラフィックからHTTP_REFERERを取除く設定をしている場合、まれに問題が発生する場合があります。 これは、フォーム使用時に画面が「固まる」現象です。例えば、ユーザがログインページで問題を抱えている場合はこの設定をNoにしてください。しかしこの設定ではブルートフォース・パスワードアタックに対して無防備な状態になります。分からない場合、この設定を「Yes」のままにしてください。';
...
$string['configslasharguments'] = 'ファイル (イメージ、アップロード等) は、スラッシュを使ってプログラムに受け渡されます。この方法により、簡単にブラウザ内やプロクシにイメージを一時保存させることが可能です。いくつかのPHPサーバではこの方法が取れない場合があります。もしファイルやイメージ (例 ユーザの顔写真等) のアップロードに問題がある場合、最初のオプションを選択してください。';
...
$string['configsmtpuser'] = '上記でSMTPサーバを設定して、なおかつ認証が必要な場合、ユーザ名とパスワードを設定してください。';
...
$string['dbmigratewarning'] = 'この処理を始める前に、Moodleデータベースをバックアップしたか確認してください。バックアップの方法が分からない場合、システム管理者にご連絡ください。移行プロセスの開始後、あなたのMoodleサイトはメンテナンスモードに変更されます。';
