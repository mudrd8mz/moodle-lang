...
$string['configsessioncookie'] = 'ここでは、Moodleのセッションで使用されるクッキー名をカスタマイズします。この設定は1つ以上のMoodleが同じウェブサイトで稼動している場合、クッキーが混乱するのを防ぐためだけに任意でお使いください。';
...
$string['configslasharguments'] = 'ファイル (イメージ、アップロード等) は、スラッシュを使ってプログラムに受け渡されます。この方法により、簡単にブラウザ内やプロクシにイメージを一時保存させることが可能です。いくつかのPHPサーバではこの方法が取れない場合があります。もし、ファイルやイメージ (例 ユーザの顔写真等) のアップロードに問題がある場合、最初のオプションを選択してください。';
...
$string['uupreprocessedcount'] = '処理レコード数: $a';
