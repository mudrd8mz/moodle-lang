<?PHP // $Id$ 
      // admin.php - created with Moodle 1.9 dev (2007040200)
...
$string['configauthenticationplugins'] = 'あなたが使用したい認証プラグインを選択して、フェイルスルーの順に並び替えてください。自己登録は、「認証」カラムで選択されたプラグインに使用されます (通常、Eメールによる自己登録)。';
...
$string['configfiltermatchonepertext'] = 'オートリンクフィルタは、各アイテムのページテキスト (例 リソース、ブロック) で最初のテキストマッチングに該当した1つのリンクのみ生成します。この設定は、「1ページあたり1回フィルタマッチする」が「Yes」の場合、無視されます。';
...
$string['configguestroleid'] = 'このロールは自動的にゲストに割り当てられます。また、このロールはパスワードなしで受講できるコースに入ったユーザに対しても一時的に割り当てられます。ロールが「moodle/レガシーロール:ゲスト」および「moodle/コース:表示」ケーパビリティを持っているか確認してください。';
$string['confighiddenuserfields'] = 'コース教師/管理者以外のユーザに隠したいユーザ情報フィールドを選択してください。この設定によりユーザのプライバシーを強化することができます。複数のフィールドは、CTRLキーを押したまま選択してください。';
...
$string['configprotectusernames'] = 'この設定を「Yes」にした場合、forget_password.phpはユーザ名またはメールアドレスを推測できるヒントを表示しません。';
