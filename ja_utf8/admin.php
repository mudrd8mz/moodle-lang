...
$string['configclamfailureonupload'] = 'アップロードファイルをclamがスキャンするよう設定しても、設定が間違っている、または不明な理由で動作しない場合は、どのように処理しますか? 「ファイルをウイルスとして扱う」を選択した場合、感染ファイルは隔離用エリアに移動されるか、削除されます。「ファイルをウイルス感染無しとして扱う」を選択した場合、通常のファイルと同じように、指定したディレクトリに移動されます。どちらの場合でも、管理者にはclamに問題がある旨が通知されます。「ファイルをウイルスとして扱う」を選択して、clamが正常に動作しない場合 ( 一般的にclamに間違ったパスを指定 )、アップロードされたすべてのファイルは、指定された隔離用エリアに移動されるか、削除されます。この設定は注意して行ってください。';
...
$string['configintroadmin'] = 'このページではサイトの全管理を行うことができる最高管理者のアカウントを作成します。正しいメールアドレスおよび安全なユーザ名、パスワードを設定してください。後でさらに管理者を作成することができます。';
...
$string['configlangmenu'] = '汎用の言語選択メニューをホームページ、ログイン画面等に表示するかどうかを選択してください。この設定は、ユーザプロフィールでユーザが表示言語を選択することに影響しません。';
$string['configlocale'] = '全体に適用されるロケールを選択してください - これは日付の書式と言語に影響します。指定した言語を表示するには、オペレーティングシステムにロケールデータがインストールされている必要があります。( 例 ja_JP.UTF-8、en_US.UTF-8、es_ES.UTF-8 ) もし分からない場合は、空白のままにしてください。';
...
$string['configsmtphosts'] = 'Moodleでメールを送信するときに使用する1つ以上のローカルSMTPサーバを入力してください ( 例 mail.a.com またはmail.a.com;mail.b.com )。空白にした場合、Moodleは自動的にPHPのメール送信機能を使用します。';
...
$string['configstatsmaxruntime'] = '統計処理は、コンピュータに多大な負荷がかかる場合があります。このオプションと次のオプションの組み合わせで、「いつ」および「どのくらい」稼動させるか設定してください。';
$string['configstatsruntimestart'] = '何時にcronジョブの統計処理を<b>開始</b>させますか?';
$string['configstatsuserthreshold'] = 'コースランキングのため、ゼロではない数値や数値ではない値をここに入力した場合、この数値より少ない登録ユーザ ( 学生 + 教師 ) のコースは無視されます。';
