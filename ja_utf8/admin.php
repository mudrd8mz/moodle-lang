...
$string['condifmodeditdefaults'] = 'ここではあなたが新しい活動を作成する場合に使用される、活動設定フォームのデフォルト値を定義します。また、どの活動設定が「高度な設定」なのか設定することもできます。';
...
$string['configdebugstringids'] = 'このオプションは翻訳者を手助けするよう、作成されました。それぞれ表示されるストリングの横に言語ファイルおよびストリングIDを表示します (この設定を変更した場合、次のページ読み込みからのみ反映されます)。';
...
$string['configenablewebservices'] = 'ウェブサービスでは他のシステムがこのMoodleにログインして、操作できるようにします。セキュリティのため、あなたが本当に使用したい場合以外、この機能を無効にしてください。';
...
$string['navigationupgrade'] = 'このアップグレードでは次のブロックに替えて、新しい2つのナビゲーションブロックが導入されます: 管理、コース、活動および参加者。これらのブロックに特別なパーミッションを設定している場合、あなたの希望するとおりに、すべてが動作するかどうか確認してください。<br /><br />また、新しいスタイルを読み込むため、あなたのブラウザを「最新の情報に更新」してください。そうでない場合、新しいブロックは正常に動作しません。';
...
$string['qtypedeletefiles'] = '問題タイプ「 $a->qtype 」に関連する、すべてのデータがデータベースから削除されました。削除を完了する (および問題タイプの再インストールを防ぐ) にはあなたのサーバより次のディレクトリを削除する必要があります: $a->directory';
