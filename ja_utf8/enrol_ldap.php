...
<p>LDAPを使用したユーザ登録を使用するには、ユーザが有効なIDナンバーフィールドを<strong>持つ必要</strong>があります。LDAPグループは、ユーザがコースに登録できるよう、メンバーフィールドの中にIDナンバーを持つ必要があります。あなたがすでにLDAP認証を使用しているのでしたら、通常これらは正常に動作します。</p>
...
$string['enrol_ldap_autocreate'] = 'Moodleに存在しないコースにユーザが登録された場合、自動的にコースを作成します。';
$string['enrol_ldap_autocreation_settings'] = 'コース自動作成設定';
$string['enrol_ldap_bind_dn'] = 'bindユーザをユーザ検索に使用したい場合、ここで指定してください。「 cn=ldapuser,ou=public,o=org 」のような設定となります。';
...
$string['enrol_ldap_course_idnumber'] = 'LDAPのユニークなidentifierにマップしてください。通常、<em>cn</em>または<em>uid</em>です。コース自動作成を使用する場合、設定値をロックしてください。';
$string['enrol_ldap_course_settings'] = 'コース登録設定';
$string['enrol_ldap_course_shortname'] = 'オプション: 「省略名」を取得するLDAPフィールドです。';
...
$string['enrol_ldap_student_contexts'] = '学生登録グループが配置されているコンテクスト一覧です。異なるコンテクストは「;」で分けてください。例: \'ou=courses,o=org; ou=others,o=org\'';
...
$string['enrol_ldap_version'] = 'あなたのサーバで使用しているLDAPプロトコルのバージョンです。';
