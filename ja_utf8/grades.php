...
$string['configbulkcheckboxes'] = 'バルク評定処理のため、評点の近くにチェックボックスを表示します。';
...
$string['configdisablegradehistory'] = '評定に関連するテーブルの変更履歴のトラッキングを無効にします。この設定により、サーバのオペレーション速度を向上させ、データベースの使用量を節約します。';
$string['configenableajax'] = '一般的な操作を単純化および高速化するAJAX機能のレイヤーを評定レポートに追加します。ユーザのブラウザレベルでJavaスクリプトが有効にされているかどうかに依存します。';
$string['configenableoutcomes'] = 'アウトカム (別名 コンピテンシー、ゴール、スタンダード、クライテリア) のサポートでは、アウトカム記述に関する１つまたはそれ以上の尺度を使用して評定項目を評定することができます。アウトカムを有効にすることで、サイト全体を通して特別な評定を可能にします。';
...
$string['configgradehistorylifetime'] = 'ここでは、評定に関連するテーブル履歴の保存期間を指定します。可能な限り長い期間の保存をお勧めします。あなたがパフォーマンスに関する問題を経験した場合、またはデータベース領域が制限されている場合、この値を小さくしてください。';
...
$string['configquickfeedback'] = '多くの評定のフィードバックを1度に編集できるようにするため、クイックフィードバックは評定レポートのそれぞれの評定セルにテキスト入力欄を追加します。1つずつ変更する代わりに、更新ボタンをクリックすることで、すべての変更を1度に実行することができます。';
$string['configquickgrading'] = '多くの評定を1度に編集できるようにするため、クイック評定は評定レポートのそれぞれの評定セルにテキスト入力欄を追加します。1つずつ変更する代わりに、更新ボタンをクリックすることで、すべての変更を1度に実行することができます。';
...
$string['configshowuseridnumber'] = '評定レポートの特別カラムにユーザIDナンバーを表示するかどうか設定します。';
$string['configshowuserimage'] = '評定レポートで名前の隣にプロファイルイメージを表示するかどうか設定します。';
...
$string['exportselectedoutcomes'] = '選択したアウトカムをエクスポートする';
...
$string['importskippednomanagescale'] = 'あなたには新しい尺度を追加するパーミッションがありません。アウトカム「 $a 」は、新しい尺度の作成をスキップしました。';
...
$string['overridesitedefaultgradedisplaytypehelp'] = '評定表の評定表示サイトデフォルトをオーバーライドしたい場合、このチェックボックスをチェックしてください。チェックボックスをチェックすることで、あなたが評定文字および境界を選択できるフォーム要素がアクティブにされます。';
