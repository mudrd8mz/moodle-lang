<?PHP // $Id$ 
      // scorm.php - created with Moodle 1.6 + (2006050506)


$string['advanced'] = 'Параметри';
$string['asset'] = 'Фактор';
$string['attempt'] = 'покушај';
$string['attempts'] = 'покушаји';
$string['attr_error'] = 'Неадекватне вредности за атрибут ($a->attr) у ознаци $a->tag.';
$string['autocontinue'] = 'Аутоматски наставак';
$string['averageattempt'] = 'Просечни број покушаја';
$string['badmanifest'] = 'Има грешака у манифесту: видите евидентиране грешке';
$string['browse'] = 'Преглед';
$string['browsed'] = 'Претражено';
$string['browsemode'] = 'Начин прегледања података';
$string['chooseapacket'] = 'Изаберите или ажурирајте SCORM пакет';
$string['completed'] = 'Завршено';
$string['coursepacket'] = 'Пакет курса';
$string['coursestruct'] = 'Структура курса';
$string['datadir'] = 'Грешка датотечног система: Није могуће креирати директоријум за податке курса';
$string['details'] = 'Прати детаље';
$string['directories'] = 'Приказ линкова директоријума';
$string['display'] = 'Приказ пакета';
$string['domxml'] = 'DOMXML екстерна библиотека';
$string['element'] = 'Елемент';
$string['entercourse'] = 'Улаз у SCORM курс';
$string['errorlogs'] = 'Евидентиране грешке';
$string['exit'] = 'Излаз из SCORM курса';
$string['expcoll'] = 'Проширивање/Скупљање';
$string['failed'] = 'Није успело';
$string['firstaccess'] = 'Први приступ';
$string['firstattempt'] = 'Први покушај';
$string['found'] = 'Манифест је нађен';
$string['frameheight'] = 'Подешена подразумевана висина оквира или прозора нивоа';
$string['framewidth'] = 'Подешена подразумевана ширина оквира или прозора нивоа';
$string['fullscreen'] = 'Попуни цео екран';
$string['general'] = 'Општи подаци';
$string['gradeaverage'] = 'Просечна оцена';
$string['gradehighest'] = 'Највиша оцена';
$string['grademethod'] = 'Начин оцењивања';
$string['gradescoes'] = 'Објекти за учење';
$string['gradesum'] = 'Коначна оцена';
$string['height'] = 'Висина';
$string['hidden'] = 'Сакривено';
$string['hidebrowse'] = 'Онемогућавање мода приказа';
$string['hidenav'] = 'Сакривање навигационих дугмади';
$string['hidetoc'] = 'Приказ структуре курса (садржаја)';
$string['highestattempt'] = 'Најбољи покушај';
$string['identifier'] = 'Идентификатор питања';
$string['iframe'] = 'Тренутни прозор';
$string['iframedisplay'] = 'Отварање пакета у тренутном прозору';
$string['incomplete'] = 'Непотпуно';
$string['interactions'] = 'Интеракције';
$string['lastaccess'] = 'Последњи приступ';
$string['lastattempt'] = 'Последњи покушај';
$string['location'] = 'Приказ локационе линије';
$string['max'] = 'Најбољи резултат';
$string['maximumattempts'] = 'Број покушаја';
$string['menubar'] = 'Приказ линије менија';
$string['min'] = 'Најгори резултат';
$string['missing_attribute'] = 'Недостаје атрибут $a->attr у ознаци $a->tag';
$string['missing_tag'] = 'Недостаје ознака $a->tag';
$string['mode'] = 'Начин рада';
$string['modulename'] = 'SCORM/AICC';
$string['modulenameplural'] = 'SCORM/AICC пакети';
$string['newattempt'] = 'Почни нови покушај';
$string['next'] = 'Наставак';
$string['no_attributes'] = 'Ознака $a->tag мора имати атрибуте';
$string['no_children'] = 'Ознака $a->tag мора имати потомке';
$string['noactivity'] = 'Нема података за извештај';
$string['nolimit'] = 'Неограничени број покушаја';
$string['nomanifest'] = 'Манифест није пронађен';
$string['noprerequisites'] = 'Нажалост, не испуњавате довољно предуслова за приступ овом објекту за учење';
$string['noreports'] = 'Нема извештаја за приказ';
$string['normal'] = 'Нормално';
$string['noscriptnoscorm'] = 'Ваш web читач не подржава Javascript или је у његовим подешавањима искључена подршка за Javascript. Није могуће забележити никакве податке о праћењу.';
$string['not_corr_type'] = 'Неслагање типова за ознаку $a->tag';
$string['notattempted'] = 'Није покушавано';
$string['objectives'] = 'Циљеви';
$string['organization'] = 'Организација';
$string['organizations'] = 'Организације';
$string['othertracks'] = 'Други записи';
$string['packagedir'] = 'Грешка датотечног система: Није могуће креирати директоријум пакета';
$string['packagefile'] = 'Није наведена датотека с пакетом';
$string['passed'] = 'Положено';
$string['php5'] = 'PHP 5 (DOMXML изворна библиотека)';
$string['popup'] = 'Нови прозор';
$string['popupmenu'] = 'У падајућем менију';
$string['popupopen'] = 'Отварање пакета у новом прозору';
$string['position_error'] = 'Ознака $a->tag не може бити подређена ознаци $a->parent';
$string['prev'] = 'Претходни';
$string['raw'] = 'Основни резултат';
$string['regular'] = 'Регуларни манифест';
$string['report'] = 'Извештај';
$string['resizable'] = 'Дозволи промену величине прозора';
$string['result'] = 'Резултат';
$string['review'] = 'Преглед';
$string['reviewmode'] = 'Мод за прегледање';
$string['scoes'] = 'Објекти за учење';
$string['score'] = 'Резултат';
$string['scormcourse'] = 'SCORM курс';
$string['scrollbars'] = 'Дозволити скроловање прозора';
$string['sided'] = 'Са леве стране';
$string['stagesize'] = 'Величина нивоа';
$string['status'] = 'Статус';
$string['statusbar'] = 'Прикажи статусну линију';
$string['student_response'] = 'Одговор';
$string['syntax'] = 'Синтаксна грешка';
$string['tag_error'] = 'Непозната ознака ($a->tag) са овим садржајем: $a->value';
$string['time'] = 'Време';
$string['too_many_attributes'] = 'Ознака $a->tag има јако пуно карактеристика';
$string['too_many_children'] = 'Ознака $a->tag има јако пуно потомака';
$string['toolbar'] = 'Прикажи линију са алаткама';
$string['totaltime'] = 'Време';
$string['trackingloose'] = 'УПОЗОРЕЊЕ: Подаци о праћењу овог SCORM пакета биће изгубљени!';
$string['type'] = 'Тип';
$string['validateascorm'] = 'Проверите исправност SCORM пакета';
$string['validation'] = 'Резултат провере исправности';
$string['validationtype'] = 'Ова опција подешава DOMXML библиотеку која се користи за валидацију SCORM манифеста. Ако нисте сигурни коју вредност треба да поставите, оставите тренутно активни избор.';
$string['value'] = 'Вредност';
$string['versionwarning'] = 'Верзија манифеста је старија од 1.3, упозорење на ознаци $a->tag';
$string['viewallreports'] = 'Приказ извештаја за $а покушаја';
$string['whatgrade'] = 'Оцењивање покушаја';
$string['width'] = 'Ширина';
$string['window'] = 'Прозор';

?>
