<?PHP // $Id$ 
      // repository_alfresco.php - created with Moodle 1.9.2+ (Build: 20080910) (2007101522)
      // local modifications from http://127.1.1.0


$string['alfresco_url'] = 'Alfresco URL-je';
$string['configplugin'] = 'Alfresco beállítása';
$string['notitle'] = 'nincs cím';
$string['password'] = 'Jelszó';
$string['repositorydesc'] = 'Segédprogram az Alfresco CMS-hez';
$string['repositoryname'] = 'Az Alfresco adattára';
$string['soapmustbeenabled'] = 'Az Alfresco segédprogramhoz be kell kapcsolni a SOAP-kiterjesztést.';
$string['username'] = 'Felhasználónév';

?>
