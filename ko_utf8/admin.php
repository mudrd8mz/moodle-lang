...
$string['configforcelogin'] = '일반적으로 이 사이트의 초기화면과 강좌 목록은 로그인 하지 않아도 사람들이 볼 수 있다. 하지만 로그인 하지 않고는 아무 것도 할 수 없게 하려면 이 설정을 \"예\"로 해 두어라.';
...
$string['configfullnamedisplay'] = '이것은 전체 이름을 표시하는 방법을 정의하는 것이다. 대부분의 한 개 언어만 쓰는 사이트에서는 기본값인 \"성+이름\"으로 충분히 효과를 볼 수 있지만, 다양한 언어팩을 쓰는 경우 특히 성이나 이름이 매우 긴 경우에는 이름을 보이지 않게 하거나 성을 보이지 않게 설정할 수도 있다.';
...
$string['configguestroleid'] = '이 역할은 손님계정에 자동으로 부여되는 것이다. 키없는 손님 방문을 허용한 강좌에 참여한 비등록 이용자에게도 한시적으로 적용된다. moodle/legacy:guest 및 moodle/course:view 권한이 부여되어 있는지 확인하라.';
...
$string['confignodefaultuserrolelists'] = '이 설정은 기본 역할이 접근을 허용함에도, 사이트 강좌에 대하여 get_course_user 등에 의해 거부된 호출을 통하여 이용자들이 데이터베이스에서 되돌아 오는 것을 막는다. 성능상 문제가 있는 경우 이것을 체크하라.';
$string['confignonmetacoursesyncroleids'] = '기본적으로 메타코스 하위 강좌의 모든 출석은 메타코스와 동기화된다. 단, 여기에서 선택된 역할들은 동기화 과정에 포함되지 않는다.';
