...
$string['adminseesownevents'] = '관리자는 일반 사용자와 동일';
...
$string['allowvisiblecoursesinhiddencategories'] = '숨겨진 범주에서 강좌 볼 수 있게 허용';
...
$string['cfgwwwrootslashwarning'] = 'config.php에&#36;CFG->wwwroot 를 잘 못 정의하였습니다. 마지막에\'/\' 문자가 들어있습니다. 이 문자를 제거하세요, 그렇지 않으면 <a href=\'http://tracker.moodle.org/browse/MDL-11061\'>MDL-11061</a> 와 같은 버그가 발생할 것입니다.';
$string['cfgwwwrootwarning'] = 'config.php에&#36;CFG->wwwroot 를 잘 못 정의하였습니다. URL 주소가 맞지 않습니다. 수정하세요, 그렇지 않으면 <a href=\'http://tracker.moodle.org/browse/MDL-11061\'>MDL-11061</a> 와 같은 버그가 발생할 것입니다.';
$string['change'] = '변경';
...
$string['configallowcoursethemes'] = '이것을 활성화하면, 각 강좌는 나름대로 독특한 테마로 설정될 수 있다. 강좌 테마는 모든 다른 테마(사이트, 사용자, 또는 세션 테마)선택보다 우선하여 적용된다';
...
$string['configallowobjectembed'] = '기본 보안 규정에 따라, 일반 사용자들은 멀티미디어 파일 (예:플래시,동영상,mp3)을 EMBED, OBJECT 등의 명령어나 태그를 사용해서 올릴수 없다. (비록 안전한 미디어 파일일지라도) 운영자가 허락 옵션을 켜야만 파일을 올릴 수 있다.';
...
$string['configallowuserthemes'] = '이것을 활성화하면, 사용자는 자신만의 테마를 사용할 수 있게 된다. 사용자 테마는 기존의 (강좌 테마가 아닌)싸이트 테마보다 우선적으로 적용된다.';
$string['configallusersaresitestudents'] = '이 사이트의 초기화면의 활동에 대하여 모든 사용자가 학생으로 간주되도록 할 것인가? 만약 당신이 \"예\"라고 설정한다면 승인된 사용자 누구라도 다른 과정에 손쉽게 학생으로 참여할 수 있게 된다. 만얀 당신이 \"아니오\"라고 한다면 사용자는 초기화면 활동의 특정 과정에만 참여하도록 제한될 것이다. 이 때는 관리자와 특별히 임명된 교수자만 초기화면에서 교수자로써 활동할 수 있다.';
...
$string['configbloglevel'] = '본 설정은 이 사이트에 있는 사용자 블로그를 어느 수준까지 보게 할 것인가를 지정하는 것이다. 블로그에 게제된 유형 혹은 내용을 지정하는 것이 아니라 Viewer의 최대 영역을 지정한다는 점에 유의하기 바란다. 여러분이 블로그 사용을 꺼려하면 이를 비활성화 시킬 수 있다.';
$string['configcachetext'] = '큰 싸이트나 문서 필터를 사용하는 싸이트라면 이 설정값을 적절히 줌으로서 처리 속도를 빠르게 할 수 있다. 문서는 여기에 지정된 시간 동안 메모리에 저장된다. 실제로 이 값을 너무 작게 설정하면 다소 처리 속도가 늦게 되며, 또 너무 큰 값을 설정해도 입력된 내용(예를 들어, 새로운 링크)을 갱신하는데 시간이 오래 걸릴 것이다.';
$string['configcachetype'] = '무들이 사용할 캐쉬 유형을 고르라. 이것은 단지 캐쉬만 설정하는 것이고 이를 사용하기 위해서는 rcache를 활성화해야 한다는 것을 기억하기 바란다. 데이터베이스 시스템의 부하를 줄일 필요가 있을 때만 사용하라. 그 외에 사용하면 무들은 더 느리게 작동할 것이다. 접속 수가 그리 많지 않은 사이트에서는 \'내부\' 캐쉬를 사용할 경우 이득을 볼 수도 있다. <em>여분 메모리가 활성화 된</em> eAccelerator 혹은 Turckmmcache 가 구동되는 단일 웹서버에서는 \'eaccelerator\'를 시도해 봄직도 하다. 만일 다중 서버로 구성되어 있고 하나 이상의 memcached 데몬이 구동되고 있으며 PHP-memcached 확장을 이용하고 있다면, \'memcached\'를 선택하고 아래에 있는 선택사항을 설정하기 바란다. <br /><strong>주의:</strong> 부하가 걸린 상태에서 성능 점검을 해 보고 적당히 조정해야 함을 명심하라. 매우 접속 수가 많은 경우에는 eAccelerator 와 memcached 는 최대의 잇점을 보장할 수 있지만, 웹서버 상 CPU 사용에 고 비용을 초래한다.';
...
$string['configcoursemanager'] = '여기에서는 강좌 설명에 표시되는 사람들을 설정할 수 있게 한다. 사용자가 강좌 설명에 표시되게 하기 위해서는 이 중 하나의 역할이 되어야 한다.';
...
$string['configdefaultcourseroleid'] = '강좌에 등록하는 사용자는 이 역활이 자동적으로 부여된다.';
$string['configdefaultrequestcategory'] = '사용자가 요청한 강좌는 자동적으로 이 범주에 속하게 된다.';
$string['configdefaultrequestedcategory'] = '승인된 교육과정이 들어갈 기본 범주';
$string['configdefaultuserroleid'] = '이곳에서 지정한 역할로 모든 사용자에게 사이트 수준의 역할이 부여된다. 또한 다른 역할도 부여할 수 있다. 기본 역할은 손님이다. 다른 역할과 상충되지 않게 주의하고, 모든 사용자가 사이트 수준에서 무언가 유용한 일(포럼의 토론 및 자료 읽기 등)을 할 수 있도록 설정하라.';
$string['configdeleteunconfirmed'] = '만약 이메일 인증제를 사용한다면, 여기에 설정된 기간내에 사용자들이 확인 응답을 해야한다. 이 기간이 끝나면 확인 응답을 하지 않은 계정은 삭제된다.';
...
$string['configdocroot'] = '무들 도움말 문서의 경로를 정의한다. 만일 나름대로의 사용자 온라인 문서를 갖고자 할 경우에는 이를 바꿀 수 있다. 그러나 문서 경로가 http://docs.moodle.org 과 같은 형식을 사용하도록 하라.';
...
$string['configenableajax'] = '만일 AJAX 기능을 활성화하면 사용자는 자신들의 개인정보에서 이를 비활성화 할 수 있으나 비활성화하면 모든 사람이 AJAX 기능을 쓸 수 없게 된다.';
...
$string['configenablerssfeedsdisabled2'] = 'RSS feed가 서버 수준에서 비활성화 되어 있습니다. 서버/RSS에서 우선 활성화 할 필요가 있습니다.';
...
$string['configforcelogin'] = '일반적으로 이 싸이트의 초기화면과 강좌 목록은 로그인 하지 않아도 사람들이 볼 수 있다. 하지만 로그인 하지 않고는 아무 것도 할 수 없게 하려면 이 설정을 \"예\"로 해 두어라.';
$string['configforceloginforprofiles'] = '이 기능은 손님계정이 아닌 실 ID로 로그인 해야만 다른 사람의 개인정보를 볼 수 있게 조정하는 것이다. 기본적으로 이 기능은 \"아니오\"로 해 놓았는데, 이러면 잠재적 학생들이 각 과정 선생님에 대한 개인정보를 살펴볼 수 있게 할 수 있으나 반면 웹 서치 엔진이 그 내용을 누출할 수도 있다.';
...
$string['configfrontpageloggedin'] = '위에서 선택된 항목은 사용자가 로그인할 때 사이트 초기화면에 표시될 것이다.';
...
$string['configguestroleid'] = '이 역할은 방문객에게 자동으로 부여되는 것이다. 키없는 손님 방문을 허용한 강좌에 방문객으로 참여한 비등록 사용자에게도 한시적으로 적용된다. moodle/legacy:guest 및 moodle/course:view 권한이 부여되어 있는지 확인하라.';
$string['confighiddenuserfields'] = '교수자와 관리자 이외의 사람들에게는 보이게 하고 싶지 않은 사용자 정보를 선택하라. 이렇게 하면 학생들의 프라이버시를 증진시킬 수 있다. 여러 항목을 선택할 때는 CTRL키를 누른채 항목들을 선택하면 된다.';
...
$string['configlangdir'] = '대부분 언어들은 왼페이지에서 오른페이지으로 인쇄되나, 아라비아어나 헤브리아어 같은 일부 언어들은 오른페이지에서 왼페이지으로 인쇄된다.';
...
예시: ko,en,fr,it';
$string['configlangmenu'] = '사이트 전체에 걸쳐 사용될 언어 선택 목록을 보이게 할 것인지 아닌지를 설정하는 것이다. 이 설정은 사용자들이 개별적으로 개인정보에서 선택한 언어에는 영향을 미치지 않는다.';
...
$string['configlongtimenosee'] = '만약 학생들이 오랫동안 로그인을 하지 않으면, 그들은 그 강좌로부터 자동으로 제명된다. 이 변수는 그 시간 한도를 규정한다.';
$string['configlookahead'] = '행사 미리 게시할 날';
$string['configmailnewline'] = '개행문자가 메일 내용에 사용되었습니다. CRLF가 RFC 822bis에 따라 필요합니다. 어떤 메일서버는 LF를 CRLF로 자동변환하는 반면 어떤 서버는 CRLF를 CRCRLF로 잘 못 변환하고, 어떤 메일서버는 LF만 있는 메일을 거절합니다(예를 들어 qmail) 배달되지 않는 메일이 있거나 이중 개행문자가 있는 경우 이 설정을 변경해 보십시요.';
...
$string['confignodefaultuserrolelists'] = '이 설정은 만일 기본 역할이 접근을 허용하는 경우 사이트 강좌에 대하여 get_course_user등의 격하된 호출로부터 사용자들이 데이터베이스에서 돌아오는 것을 못하게 합니다. 성능상 문제를 초래하는 경우 이것을 체크하십시요.';
$string['confignonmetacoursesyncroleids'] = '기본적으로 하위 강좌의 모든 출석은 메타코스와 동기화된다. 여기에서 선택된 역할들은 동기화 과정에 포함되지 않는다.';
...
$string['configproxypassword'] = '필요한 경우 프록시를 통해 인터넷에 접속하는데 암호가 필요합니다. 필요하지 않은 경우 공백(PHP cURL 익스텐션이 필요)';
...
$string['configproxyuser'] = '필요한 경우 프록시를 통해 인터넷에 접속하는데 사용자 이름이 필요합니다. 필요하지 않은 경우 공백(PHP cURL 익스텐션이 필요)';
...
$string['configrestrictbydefault'] = '윗단계 범주에서 설정된 모듈을 새로운 강좌의 기본 모듈이 되도록 제한할 것인가?';
$string['configrestrictmodulesfor'] = '교육과정에 몇몇의 활동 모듈을 사용안함으로 설정해 놓을 것인가? 이 설정은 오로지 교수자에게만 적용되고, 관리자는 여전히 강좌에 어떠한 활동이라도 첨삭할 수 있음에 유의하라.';
...
$string['configshowblocksonmodpages'] = '어떤 활동 모듈은 화면에 블록이 생성될 수 있도록 지원한다. 만약 이 설정을 \"예\"로 놓으면, 교수자들은 사이드 블록을 추가시킬 수 있을 것이다. \"아니오\"로 놓으면, 아예 사이드 블록 설정 기능을 보여주지 않는다.';
...
$string['configtimezone'] = '여기에서 표준시간대를 선택한다. 이것은 단지 기본값으로 쓸 표준시간대이며, 각 사용자는 개인정보 난에서 그들 자신에 맞는 설정을 따로 할 수가 있다. 이곳의 \"서버 로칼 시간대\"는 서버 전반에 적용되는 기준 시간으로 무들의 기본값인데, 사용자 개인정보에 있는 시간대와는 별도이다. Cronjob은 이 시간을 기준으로 작동한다.';
...
$string['defaultcourseroleid'] = '강좌의 사용자 기본 역활';
...
$string['enablerssfeeds'] = 'RSS Feed 활성화';
...
$string['forceloginforprofiles'] = '개인정보를 위해 강제로 로그인 하게 함';
...
$string['frontpageloggedin'] = '로그인 했을 때의 초기화면 항목';
$string['frontpagequestions'] = '초기화면 질문';
...
$string['langpackinstalled'] = '언어팩 $a이 성공적으로 설치되었습니다.';
...
$string['langpackupdateskipped'] = '$a 언어팩 갱신이 생략되었습니다.';
$string['langpackwillbeupdated'] = '번역자들을 위한 안내: 업그레이드 도중 무들은 언어팩을 갱신할 것입니다. _local 폴더 외에 있는 변경사항은 덮어쓰게 될 것입니다.';
$string['langrmyourself'] = '데이터 손실을 방지하기 위해서 lang.php파일은 비어있는 내용으로 존재하는 파일을 덮어쓰기 할 수 없습니다. 파일을 제거하기 위해서는 수동으로 파일을 제거하세요.';
...
$string['mail'] = '전자우편';
$string['mailnewline'] = '메일에서 새로운 라인 문자';
...
$string['messaging'] = '내부 페이지글 발송 가능';
...
$string['pleaserefreshregistration'] = '당신의 사이트는 moodle.org에 등록이 되어 있습니다. $a에 최근 갱신이후 상당한 변화가 있었다면 등록을 갱신하는 것을 고려해 보시기 바랍니다.';
...
$string['requiredtemplate'] = '필요함. 템플릿 문법을 사용할 수 있습니다 (%%l = lastname, %%f = firstname, %%u = username). 세부사항과 예제는 도움말을 보세요.';
...
$string['timezonenotforced'] = '나름대로의 표준시를 사용할 수 있습니다.';
...
$string['uploaduserspreview'] = '사용자 미리보기 업로드';
$string['uploadusersresult'] = '사용자 결과 업로드';
$string['upwards'] = '위로';
$string['usehtmleditor'] = 'HTML편집기 사용';
$string['useraccountupdated'] = '사용자가 갱신됨';
$string['userbulk'] = '일괄사용자처리';
...
$string['uuoptype_addinc'] = '모두 추가, 필요한 경우 사용자 이름에 계수기 추가';
$string['uuoptype_addnew'] = '새 사용자만 추가, 기존 사용자 생략';
$string['uuoptype_addupdate'] = '새 사용자 추가 및 기존 사용자 갱신';
$string['uuoptype_update'] = '기존 사용자만';
