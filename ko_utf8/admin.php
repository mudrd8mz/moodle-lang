<?PHP // $Id$ 
      // admin.php - created with Moodle 1.6 Beta 3 (2006042400)
...
$string['badwordsconfig'] = '잘못된 단어들을 콤마로 구분해 입력하시오';
...
$string['configbloglevel'] = '본 설정은 이 사이트에 있는 사용자의 블로그를 어느 수준까지 볼 것인가를 지정하는 것입니다. 블로그에 게제된 유형 혹은 내용을 지정하는 것이 아니라 Viewer의 최대 내용을 지정한다는 점에 유의하시기 바랍니다. 여러분이 사용하길 꺼려하면 블로그를 비활성시킬 수 있습니다.';
...
$string['configdocroot'] = '무들 문서로의 경로를 정의합니다. 당신이 사용자 온라인 문서를 갖고자하는 경우에는 이것을 바꿀 수 있습니다. 그러나 당신의 문서에서 경로가 http://docs.moodle.org 에서와 같은 형식을 사용하도록 하십시요.';
...
$string['configenrolmentplugins'] = '당신이 사용하고자 하는 등록 플러그인을 선택하십시요. 설정을 하는 것을 잊지 마십시요.<br /><br />어느 플러그인이 활성화 되어 있는지 나타내기 바랍니다.<strong>하나의</strong> 플러그인이 <em>상호작용적</em> 등록에 필요한 기본 플로그인으로 설정될 수 있습니다. 상호작용적 등록을 비활성화 하기 위해서는 필요한 배움터에서 \"등록가능\" 을 \"아니오\"로 하십시요.';
...
$string['configsessioncookiepath'] = '브라우저가 무들 쿠키를 전송하는 위치를 변경하려면 이 곳에 특정 디렉토리를 설정하면 됩니다. 그냥 두면 기본값인 \'/\'로 설정되게 됩니다.';
...
$string['cronwarning'] = '<a href=\"cron.php\">cron.php 관리 스크립트</a>가 최소 24시간 동안 작동 되지 않습니다';
$string['dbmigrate'] = '무들 데이터베이스 변경';
$string['dbmigrateconnecerror'] = '지정한 데이터 베이스에 연결할 수 없습니다.';
$string['dbmigrateencodingerror'] = '$a로 인코드된 데이터베이스명보다는 UNICODE/UTF8을 쓰기 바랍니다.<br />다시 지정하기기 바랍니다.';
...
$string['dbmigratewarning2'] = '<b>경고 : 데이터베이스 이전과정을 시작하려 합니다. 다시한번 무들의 데이터베이스를 백업해 두었는지 확인하기 바랍니다.</b>';
...
$string['dbmigrationdupfailed'] = '데이터베이스 복제 실패. 가능한 오류: <font color=\"#ff0000\"><pre>$a</pre></font>';
$string['density'] = '밀도';
...
$string['importlangreminder'] = '데이터베이스 마이그레이션 프로세스가 시작되었습니다. 업그레이드 효과가 나타나기 위해서는 <b>다시 로그인</b>을 해야 합니다. 데이터베이스 마이그레이션 과정이 끝나면 관리->언어->언어 가져오기 도구로부터 유니코드 언어팩을 설치하십시요.';
$string['importtimezones'] = '시간대역 목록의 업데이트가 완료되었습니다.';
...
$string['pathtopgdumpdesc'] = '당신의 시스템에 한개 이상의 pg_dump가 있을 경우에(예를 들어 postgresql 버전을 여러개 설치된 경우)만 필요합니다.';
$string['pathtopgdumpinvalid'] = 'pg_dump로의 잘못된 경로- 경로가 잘못되었거나 실행화일이 아닙니다.';
$string['pathtopsql'] = 'psql  경로';
$string['pathtopsqldesc'] = '당신의 시스템에 한개 이상의 psql이 있을 경우에(예를 들어 postgresql 버전이 여러개 설치된 경우)만 필요합니다.';
$string['pathtopsqlinvalid'] = 'psql 에로의 잘못된 경로- 경로가 잘못되었거나 실행화일이 아닙니다.';
$string['pgcluster'] = 'PostgreSQL 클러스터';
$string['pgclusterdescription'] = '명령어 라인 실행을 위한 PostgreSQL 버전/클러스터 파라메터. 만일 한개의 postgresql이 설치되어 있다거나 잘 모를경우에는 공백으로 남겨놓으십시요.';
...
$string['unicodeupgradenotice'] = '무들 1.6에서는 모든 언어가 유니코드로 바뀌었습니다. 이사이트를 업그레이드하기 위해서는 다음 변환 스크립트  <a href=\"utfdbmigrate.php\">변환스크립트를 사용하기 위해서는 여기를 클릭하세요</a>를 사용하여 데이터베이스의 모든 데이터를 변환하여야 합니다.';
...
$string['usersrenamed'] = '사용자이름들이 변경됨';
