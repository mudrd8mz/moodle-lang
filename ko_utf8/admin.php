<?PHP // $Id$ 
      // admin.php - created with Moodle 1.7 dev (2006081000)
...
$string['cachetext'] = '텍스트 기억 메모리 수명';
...
$string['configdbsessions'] = '이를 켜 놓으면, 현재 세션의 정보를 저장하기 위해서 데이타 베이스를 사용할 것이다. 이것은 큰 싸이트나 많은 서버를 포함한 사이트에서 특히 유용하게 쓰일 수 있다. 대부분의 싸이트에서는 이를 막아 놓아, 서버 디스크에 세션 정보를 기록하도록 한다. 만일 지금 이 옵션을 바꾸게 되면, 현재 접속된 모든 사용자(당신도 물론!)의 접속이 끊어져 로그아웃될 것이다.';
...
$string['configintrosite'] = '이 화면에서는 이 싸이트의 명칭과 첫 화면에 대한 설정을 할 수 있다. 여러분은 언제라도 다시 홈 페이지의 관리 > 환경설정 > 사이트 설정 항목을 통해 이 곳으로 돌아와 이 설정들을 바꿀 수 있다.';
...
$string['configlangcache'] = '언어 메뉴의 임시 저장. 많은 메모리가 절약되고 처리 속도가 빨라 질 것이다. 만약 이것을 설정한다면. 당신이 언어를 추가하거나 삭제한 후, 메뉴는 수 분 이내에 업데이트 될 것이다.';
...
$string['confignoreplyaddress'] = '이메일은 때때로 사용자 자신에게 보내집니다. 여기에 적은 이메일 주소는 발신자가 일일이 수신자로부터 답신을 받을 수 없는 경우에 사용되는 관리용 발신 전용의 주소입니다. 
...
$string['configopentogoogle'] = '만약 여러분이 이 절정을 \"예\"로 놓으면 구글은 너의 사이트에 손님계정으로 들어올 수 있게 된다. 또한, 구글서치를 통해서 들어오는 사람들도 자동적으로 손님 계정으로 로그인 될 것이다. 또 이미 손님계정의 방문을 허락한 곳이라면 구글은 아무 제한없이 투명하게 접속시킨다는 점을 유의하라.';
...
$string['configslasharguments'] = '파일들은 슬래쉬 아규먼트를 사용하는 스크립트를 통해서 제공된다. 이 방법은 파일들이 웹 브라우저나 프록시 서버등에서 훨씬 쉽게 저장되도록 한다. 불행하게도, 어떤 PHP서버들은 이 방법을 허용하지 않는다. 그래서 만약 업로드된 파일들이나 이미지들을 보는데 문제가 있다면, 이 변수를 처음 옵션으로 셋팅하라.';
...
$string['configzip'] = '당신의 zip 프로그램의 위치(유닉스만,선택사항)를 적어라. 만약 지정되면, 이것은 서버상에서 zip 문서를 생성시킬 것이다. 만약 당신이 이 빈칸을 비워둔다면, 무들은 내부 루틴을 사용할 것이다.';
...
$string['langimport'] = '언어팩 가져오기 도구';
$string['langimportsuccess'] = '언어팩 업그레이드 완료';
...
$string['stickyblocksmymoodle'] = '개인학습방';
$string['stickyblockspagetype'] = '환경설정 페이지 양식';
$string['tabselectedtofront'] = 'tab이 있는 테이블에서 현재 선택되어진 태그가 있는 줄은 제일 앞에 위치 해 있어야만 합니다.';
