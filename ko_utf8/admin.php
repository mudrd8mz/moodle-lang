...
$string['allowuserblockhiding'] = '이용자에게 블록감추기 허용';
...
$string['configallowuserblockhiding'] = '이용자들이 이 사이트의 블록을 숨기거나 볼 수 있도록 허용하겠는가? 이 기능은 각각의 분리된 블록을 기억하기 위해 자바 스크립트와 쿠키를 사용하며 이용자 화면에만 영향을 미친다.';
$string['configallowusermailcharset'] = '이 기능을 활성화시키면, 모든 이용자는 이메일에 자신만의 문자셋을 이용할 수 있다.';
$string['configallowuserthemes'] = '이것을 활성화하면, 이용자는 자신만의 테마를 사용할 수 있게 된다. 개인별 테마는 기존의 (강좌 테마가 아닌)사이트 테마보다 우선적으로 적용된다.';
$string['configallusersaresitestudents'] = '이 사이트 초기화면의 활동에 대하여 모든 이용자가 학생으로 간주되도록 할 것인가? 만약 \"예\"라고 설정한다면 승인된 이용자 누구라도 다른 과정에 손쉽게 학생으로 참여할 수 있게 된다. 만얀 당신이 \"아니오\"라고 한다면 이용자는 초기화면 활동의 특정 과정에만 참여하도록 제한될 것이다. 이 때는 관리자와 지정된 선생님만 초기화면의 교수자로서 활동할 수 있다.';
...
$string['configcoursemanager'] = '여기에서는 강좌 설명 하단에 표시하고 싶은 사람을 설정할 수 있게 한다. 다음 중 기표된 역할이 주어진 이용자는 강좌 설명에 이름이 제시된다.';
...
$string['configdbsessions'] = '이를 켜 놓으면, 현재 세션의 정보를 저장하기 위해서 데이타 베이스를 사용할 것이다. 이것은 큰 사이트나 많은 서버로 구성된 사이트에서 특히 유용하게 쓰일 수 있다. 대부분의 사이트에서는 이를 막아 놓아, 서버 디스크에 세션 정보를 기록하도록 한다. 만일 지금 이 옵션을 바꾸게 되면, 현재 접속된 모든 이용자(당신도 물론!)의 접속이 끊어져 로그아웃될 것이다.';
...
$string['configdefaultcourseroleid'] = '강좌에 등록하는 모든 이용자는 자동적으로 이 역활이 부여된다.';
...
$string['configdefaultuserroleid'] = '이곳에서 지정한 역할로 모든 이용자에게 사이트 수준의 역할이 부여된다. 또한 다른 역할도 부여할 수 있다. 기본 역할은 손님이다. 다른 역할과 상충되지 않게 주의하고, 모든 이용자가 사이트 수준에서 무언가 유용한 일(포럼의 토론 및 자료 읽기 등)을 할 수 있도록 설정하라.';
...
$string['configdocroot'] = '무들 도움말 문서의 경로를 정의한다. 만일 나름대로의 이용자 온라인 문서를 갖고자 할 경우에는 이를 바꿀 수 있다. 그러나 문서 경로가 http://docs.moodle.org 과 같은 형식을 사용하도록 하라.';
...
$string['configenablehtmlpurifier'] = '신뢰할 수 없는 문서의 악성코드를 제거하기 위해서는 KSES 대신에 HTML 필터를 사용하라.  HTML 필터는 심혈을 기울여 개발되었고 상당한 신뢰도를 가지지만 자원 집약적인 측면이 있다. 최종 결과물은 다소 원안과 다르게 보일 수도 있다. embed 및 object 태그는 처리할 수 없으며 MathML과 오래된 lang 태그는 지원하지 않음을 유념하기 바란다.';
...
$string['configextendedusernamechars'] = '이 설정을 활성화시켜 놓으면 학생들은 사용자 아이디에 어떤 문자도 쓸 수 있다.(그들의 실제적 이름과는 관계가 없음을 주목하라) 기본적으로는 비활성화되어 있어 사용자 ID에 영문자와 숫자를 조합한 것만 허용된다.';
$string['configfilterall'] = '이 설정은 필터를 통해 머릿말, 제목, 주소창 등에 포함된 모든 문자열을 검색한다. 대개 다국어 여과장치를 쓸 때에 유용하지만, 사이트에 다소 추가적인 부담을 주게 된다.';
$string['configfiltermatchoneperpage'] = '이 자동 링크 필터는 한 페이지내의 텍스트에 나온 최초 한 단어에만 자동 링크를 걸고 나머지 단어들은 무시한다.';
$string['configfiltermatchonepertext'] = '이 자동 링크 필터는 한 문서(예: 자원 블록 등)중 첫번째 단어에 자동 링크를 시키고 나머지 것들은 모두 무시한다. 이 설정은 페이지별 필터 설정이 \"예\"이면 무시된다.';
...
$string['configforceloginforprofiles'] = '이 기능은 손님계정이 아닌 실 ID로 로그인 해야만 다른 사람의 개인정보를 볼 수 있게 조정하는 것이다. 기본적으로 이 기본값은 \"예\"이다.';
...
$string['configfrontpageloggedin'] = '위에서 선택된 항목은 이용자가 로그인할 때 사이트 초기화면에 표시될 것이다.';
...
$string['confightmleditor'] = '사이트 전반에 걸쳐 내장된 HTML 문서 편집기를 사용할 수 있게 할 것인가를 정하는 선택사항이다. 여기서 사용을 허용하더라도 사용자 입장에서는 웹브라우저의 기능에 따라 사용가능 여부가 결정될 것이다. 이용자도 스스로 사용여부를 선택할 수 있다.';
...
$string['configlang'] = '전체 사이트를 위한 기본 언어를 선택해라. 이용자들은 후에 이 설정에 대해 개별적으로 바꿀 수도 있다.';
...
$string['configlanglist'] = '이용자들이 무들에 설치된 언어팩이면 어떤 언어든지 선택할 수 있도록 가급적 이 곳을 빈칸을 남겨두어라. 그러나, 이용자들이 선택하는 언어 목록을 짧게 하고 싶으면 해당하는 언어코드를 쉼표로 분리하여 작성하여 넣으면 된다.
예시: ko,en,fr,it';
$string['configlangmenu'] = '사이트 전체에 걸쳐 사용될 언어 선택 목록을 보이게 할 것인지 아닌지를 설정하는 것이다. 이 설정은 이용자들이 개별적으로 개인정보에서 선택한 언어에는 영향을 미치지 않는다.';
...
$string['configloglifetime'] = '이것은 이용자의 활동에 관한 기록을 얼마나 오래 보유할 것인가를 정하는 것이다. 여기에 표시된 시간보다 오래된 기록은 자동적으로 삭제된다. 필요하다면 가능한한 오래 유지하는 것이 가장 좋은 방법이다. 하지만 서버의 부하가 많고, 시스템 성능이 낮다면, 기록 보유 기간을 짧게 하는 편이 더 나을지도 모른다.';
...
$string['confignodefaultuserrolelists'] = '이 설정은 만일 기본 역할이 접근을 허용하는 경우 사이트 강좌에 대하여 get_course_user등의 격하된 호출로부터 이용자들이 데이터베이스에서 돌아오는 것을 못하게 합니다. 성능상 문제를 초래하는 경우 이것을 체크하십시요.';
$string['confignonmetacoursesyncroleids'] = '기본적으로 하위 강좌의 모든 출석은 메타코스와 동기화된다. 여기에서 선택된 역할들은 동기화 과정에 포함되지 않는다.';
$string['confignoreplyaddress'] = '이메일은 때때로 게시자 자신에게도 보내진다. 여기에 적은 이메일 주소는 발신자가 일일이 수신자로부터 답신을 받을 필요가 없는 경우에 사용되는 관리용 발신 전용의 주소이다. 
(예를 들어 이용자가 자신의 이메일을 공개하지 않은 경우)';
...
$string['defaultcourseroleid'] = '강좌의 이용자 기본 역활';
$string['defaultrequestcategory'] = '강좌 요청시 기본 범주';
$string['defaultsettinginfo'] = '기본: $a';
$string['defaultuserroleid'] = '모든 이용자에게 적용되는 기본 역할';
...
$string['nodefaultuserrolelists'] = '모든 기본 역할 이용자는 표시하지 마십시요';
