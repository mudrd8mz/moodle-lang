...
$string['cfgwwwrootslashwarning'] = 'config.php에 &#36;CFG->wwwroot 를 잘못 정의하였습니다. 마지막에\'/\' 문자가 들어있습니다. 이 문자를 제거하세요, 그렇지 않으면 <a href=\'http://tracker.moodle.org/browse/MDL-11061\'>MDL-11061</a> 과 같은 버그가 발생할 것입니다.';
$string['cfgwwwrootwarning'] = 'config.php에 &#36;CFG->wwwroot 를 잘못 정의하였습니다. URL 주소가 맞지 않습니다. 수정하세요, 그렇지 않으면 <a href=\'http://tracker.moodle.org/browse/MDL-11061\'>MDL-11061</a> 과 같은 버그가 발생할 것입니다.';
$string['change'] = '변경';
...
$string['configallusersaresitestudents'] = '이 사이트 초기화면의 활동에 대하여 모든 이용자가 학생으로 간주되도록 할 것인가? 만약 \"예\"라고 설정한다면 승인된 이용자 누구라도 다른 과정에 손쉽게 학생으로 참여할 수 있게 된다. 만얀 \"아니오\"를 선택한다면 이용자는 초기화면 활동의 특정 과정에만 참여하도록 제한될 것이다. 이 때는 관리자와 지정된 선생님만 초기화면의 교수자로서 활동할 수 있다.';
...
$string['configdbsessions'] = '이를 켜 놓으면, 현재 세션의 정보를 저장하기 위해서 데이타 베이스를 사용할 것이다. 이것은 큰 사이트나 많은 서버로 구성된 사이트에서 특히 유용하게 쓰일 수 있다. 대부분의 사이트에서는 이를 막아 놓고 서버 디스크에 세션 정보를 기록하도록 한다. 만일 지금 이 옵션을 바꾸게 되면, 현재 접속된 모든 이용자(당신도 물론!)의 접속이 끊어져 로그아웃될 것이다.';
...
$string['configgradeexport'] = '성적을 내보낼 때 우선적으로 사용할 성적표 형식을 선택하라. 선택된 플러그인은 설정된 후 모든 성적에 \"최근 내보냄\" 항목을 사용할 것이다. 예를 들면 내보내기 한 레코드들은 \"새로운\" 혹은 \"새로고침\"된 것으로 간주될 것이다. 확신이 들지 않으면 모든 것을 비워 두어라.';
...
$string['confighideactivitytypenavlink'] = '활동모듈에 대하여 표시된 네비게이션에서 활동 형식(예-퀴즈) 링크를 비공개할 대상 선택';
...
$string['configiplookup'] = '여러 기록에서 (34.12.222.93 같은) IP주소를 클릭하여 IP가 어디에 속해 있는지를 추측할 수 있는 정보를 볼 수 있다. 선택할 수 있는 서로 다른 많은 플러그인이 있는데 나름대로 일장 일단을 갖고 있다.';
