<?PHP // $Id$ 
      // lesson.php - created with Moodle 1.7 dev (2006081000)


$string['accesscontrol'] = '접근 제한';
$string['actionaftercorrectanswer'] = '답 맞힌 후 할 일';
$string['activitylink'] = '활동 연결(링크)';
$string['activitylinkname'] = '$a 로 가기';
$string['addabranchtable'] = '갈래표 추가';
$string['addanendofbranch'] = '갈래끝 추가';
$string['addaquestionpage'] = '질문쪽 추가';
$string['addcluster'] = '질문묶음 추가';
$string['addedabranchtable'] = '갈래표 추가됨';
$string['addedanendofbranch'] = '갈래끝 추가됨';
$string['addedaquestionpage'] = '질문쪽 추가됨';
$string['addedcluster'] = '질문묶음 추가됨';
$string['addedendofcluster'] = '질문묶음끝 추가됨';
$string['addendofcluster'] = '질문묶음끝 추가';
$string['anchortitle'] = '학습의 시작지점';
$string['and'] = '그리고';
$string['answer'] = '답안';
$string['answeredcorrectly'] = '정답을 맞췄음';
$string['answersfornumerical'] = '수에 관한 질문은 최소 값과 최대 값을 함께 적어야 합니다.';
$string['arrangebuttonshorizontally'] = '슬라이드 쇼 모드에 갈래버튼을 수평으로 배열할까요?';
$string['attempt'] = '$a 번째 시도';
$string['attempts'] = '시도';
$string['attemptsremaining'] = '당신은 $a 번의 시도가 남아 있습니다.';
$string['available'] = '학습 시작';
$string['averagescore'] = '평균 점수';
$string['averagetime'] = '평균 시간';
$string['branchtable'] = '갈래표';
$string['cancel'] = '취소';
$string['canretake'] = '$a 은 다시 풀 수 있음';
$string['casesensitive'] = '대소문자 구별';
$string['checkbranchtable'] = '갈래표를 점검하시오';
$string['checkedthisone'] = '이미 점검하였음';
$string['checknavigation'] = '진행 점검';
$string['checkquestion'] = '문항 점검';
$string['classstats'] = '학급 통계';
$string['clicktopost'] = '고득점 목록에 점수를 입력하려면 여기를 클릭하시오';
$string['clusterjump'] = '질문묶음 내에서 보지 않은 문항';
$string['clustertitle'] = '질문묶음';
$string['collapsed'] = '제목 펼침';
$string['comments'] = '당신의 평';
$string['commentswithname'] = '$a->firstname {$a->lastname} 의 평';
$string['completed'] = '완성됨';
$string['completederror'] = '학습 완료';
$string['completethefollowingconditions'] = '진행하기 위해서는 먼저 <b>$a</b> 수업의 다음 조건들을 충족시켜야만 합니다.';
$string['conditionsfordependency'] = '선결 조건';
$string['confirmdeletionofthispage'] = '페이지 삭제 확인';
$string['congratulations'] = '축하합니다 - 마지막 학습 내용 입니다.';
$string['continue'] = '계속';
$string['continuetoanswer'] = '답 수정 계속하기';
$string['correctanswerjump'] = '정답인 경우 이동할 곳';
$string['correctanswerscore'] = '정답에 대한 점수';
$string['correctresponse'] = '정답에 대한 반응';
$string['customscoring'] = '성적매기기';
$string['deadline'] = '마감시간';
$string['defaultessayresponse'] = '선생님이 당신의 서술 내용을 평가할 것입니다.';
$string['deletedefaults'] = '$a x 기본 레슨이 삭제됨';
$string['deletedpage'] = '쪽 삭제';
$string['deleting'] = '삭제하기';
$string['deletingpage'] = '$a 쪽 삭제';
$string['dependencyon'] = '선행 조건';
$string['description'] = '서술';
$string['detailedstats'] = '자세한 통계';
$string['didnotanswerquestion'] = '이 질문에 답하지 않았음';
$string['didnotreceivecredit'] = '인정을 받지 못 했음';
$string['displayhighscores'] = '고득점 표시';
$string['displayinleftmenu'] = '왼쪽 메뉴에 보일까요?';
$string['displayleftif'] = '만일 $a 가 다음 점수보다 높다면 표시하시오 :';
$string['displayleftmenu'] = '왼쪽 메뉴 표시';
$string['displayofgrade'] = '점수 표시 (학생 전용)';
$string['displayreview'] = '복습 버튼 표시';
$string['displayscorewithessays'] = '당신은 $a->tempmaxgrade 점 중 $a->score 점을 얻었습니다.<br> 당신의 $a->essayquestions  서술형 답안(들)은 추후에 채점될 것이며 최종 점수에 추가될 것입니다.<br/> 서술형 질문(들)을 제외한 현재 점수는 $a->grade 점 중에서 $a->score 점을 받았습니다.';
$string['displayscorewithoutessays'] = '당신의 점수는 $a->score 점 입니다.($a->grade 점 만점)';
$string['editlesson'] = '$a 편집';
$string['editlessonsettings'] = '수업 환경 설정';
$string['editpagecontent'] = '쪽 내용 편집';
$string['email'] = '이메일';
$string['emailallgradedessays'] = '모든 서술형 성적을 기록한 이메일';
$string['emailgradedessays'] = '서술형 성적을 기록한 이메일';
$string['emailsuccess'] = '이메일 발송 완료';
$string['endofbranch'] = '갈래 끝';
$string['endofclustertitle'] = '질문묶음 끝';
$string['endoflesson'] = '학습 종료';
$string['enteredthis'] = '입장했습니다.';
$string['entername'] = '고득점 목록에 쓸 별명을 입력하세요';
$string['enterpassword'] = '비밀번호를 입력하세요 :';
$string['eolstudentoutoftime'] = '주의: 당신의 학습시간이 종료되었습니다.
마지막 답변이 종료 후에 입력되었다면, 그것은 평가 되지 않을 것입니다.';
$string['eolstudentoutoftimenoanswers'] = '당신은 답변을 전혀 하지 않았습니다. 
이번 학습에서 0점을 얻게 되었습니다.';
$string['essay'] = '주관식 서술';
$string['essayemailsubject'] = '$a 질문에 대한 성적';
$string['essays'] = '서술내용';
$string['essayscore'] = '서술문항 점수';
$string['fileformat'] = '파일 형태';
$string['firstanswershould'] = '첫 대답은 \"맞음\" 쪽으로 이동해야 합니다.';
$string['firstwrong'] = '답이 틀렸기 때문에 점수를 얻을 수 없습니다. 그냥 재미로 계속 해보겠습니까?(맞아도 점수 추가는 없습니다.)';
$string['flowcontrol'] = '흐름 조절';
$string['full'] = '내용 전체 펼침';
$string['general'] = '일반적';
$string['grade'] = '점수';
$string['gradebetterthan'] = '성적(&#37;)을 보다 좋게';
$string['gradebetterthanerror'] = '$a 퍼센트 보다 좋은 성적 얻기';
$string['gradeessay'] = '서술문항에 대한 점수 ($a->notgradedcount 등급 없음, $a->notsentcount 제출하지 않았음)';
$string['gradeis'] = '$a 점';
$string['gradeoptions'] = '등급 옵션';
$string['handlingofretakes'] = '학습 다시하기';
$string['havenotgradedyet'] = '아직 점수를 내지 않았음';
$string['here'] = '이 곳';
$string['highscore'] = '고득점';
$string['highscores'] = '고득점들';
$string['hightime'] = 'High time';
$string['importcount'] = '$a 문제 가져오기';
$string['importppt'] = '파워포인트 가져오기';
$string['importquestions'] = '질문 가져오기';
$string['insertedpage'] = '쪽 삽입';
$string['jump'] = '건너뛰기';
$string['jumptsto'] = '건너뛸 곳:';
$string['leftduringtimed'] = '당신은 규정된 학습시간에 자리를 비웠습니다.<br />
학습을 다시 시작하려면 계속 버튼을 눌러주세요.';
$string['leftduringtimednoretake'] = '당신은 규정된 학습시간에 자리를 비웠기 때문에<br />재학습을 하거나 계속할 수 없습니다.';
$string['lessonclosed'] = '이 학습은 $a 에 종료되었습니다.';
$string['lessoncloses'] = '학습 종료';
$string['lessondefault'] = '이 학습 환경 설정을 기본으로 씀';
$string['lessonformating'] = '학습 형태';
$string['lessonmenu'] = '학습 메뉴';
$string['lessonnotready'] = '이 학습은 진행할 수 없습니다. $a 에게 연락하기 바랍니다.';
$string['lessonopen'] = '이 학습과정은 $a 에 열릴 것입니다.';
$string['lessonopens'] = '학습 시작';
$string['lessonpagelinkingbroken'] = '학습의 시작 쪽이 발견되지 않습니다. 학습의 연결 고리가 끊어졌을 수 있습니다. 관리자에게 연락하십시요.';
$string['lessonstats'] = '학습 통계';
$string['loginfail'] = '로그인을 실패했습니다, 다시 시도하세요.';
$string['lowscore'] = '낮은 점수';
$string['lowtime'] = '낮은 시간(Low time)';
$string['mainmenu'] = '배움터 메뉴';
$string['manualgrading'] = '서술문항 채점';
$string['matchesanswer'] = '답과 일치';
$string['maxhighscores'] = '고득점부터 보기';
$string['maximumnumberofanswersbranches'] = '답변/갈래의 최대 수';
$string['maximumnumberofattempts'] = '최대 시도 수';
$string['maximumnumberofattemptsreached'] = '최대 허용 시도횟수에 도달하였습니다. 다음 쪽으로 갑니다.';
$string['maxtime'] = '제한 시간(분)';
$string['maxtimewarning'] = '학습을 마치기까지 $a 분 남았습니다.';
$string['mediaclose'] = '종료 버튼 보이기';
$string['mediafile'] = '미디어파일 창';
$string['mediafilepopup'] = '이곳을 누르면 수업 동영상 파일을 볼수 있습니다.';
$string['mediaheight'] = '창의 높이 :';
$string['mediawidth'] = '창의 폭 :';
$string['minimumnumberofquestions'] = '질문의 최소 갯수';
$string['modattempts'] = '학생들에게 복습 허용';
$string['modattemptsnoteacher'] = '복습과정은 학생에게만 해당됨';
$string['modulename'] = '완전학습';
$string['modulenameplural'] = '완전학습';
$string['movedpage'] = '쪽 옮겨짐';
$string['movepagehere'] = '이곳으로 쪽 옮김';
$string['moving'] = '$a 쪽 옮김';
$string['multianswer'] = '다답';
$string['multipleanswer'] = '복수 답';
$string['nameapproved'] = '승인된 이름';
$string['namereject'] = '죄송합니다. 당신의 이름은 필터에 의해 거부되었습니다. <br />다른 이름을 입력하세요.';
$string['nextpage'] = '다음 쪽';
$string['noanswer'] = '답을 하지 않았습니다. 되돌아 가서 답을 입력하세요.';
$string['noattemptrecordsfound'] = '한 번도 시도한 기록이 없습니다. : 주어진 점수가 없습니다.';
$string['nocommentyet'] = '아직 코멘트 없음.';
$string['nocoursemods'] = '아무런 활동도 하지 않음';
$string['noessayquestionsfound'] = '이 학습에는 서술형 문항이 없음.';
$string['nohighscores'] = '높은 점수 없음';
$string['nolessonattempts'] = '이 학습에 대해 아무런 시도도 없었음';
$string['nooneansweredcorrectly'] = '아무도 정답을 맞추지 못했음.';
$string['nooneansweredthisquestion'] = '아무도 이 질문에 답하지 않음.';
$string['noonecheckedthis'] = '아무도 이것을 확인하지 않음.';
$string['nooneenteredthis'] = '아무도 여기에 들어오지 않음';
$string['noonehasanswered'] = '아직 아무도 서술형 문항에 답하지 않았음.';
$string['noretake'] = '당신은 이 학습을 다시 하도록 허가받지 못했습니다.';
$string['normal'] = '정상 - 학습 과정 따라가기';
$string['notcompleted'] = '완성되지 않음';
$string['notdefined'] = '정의 되지 않음';
$string['nothighscore'] = '고득점 $a 목록에 등재되지 않았음';
$string['notitle'] = '제목없음';
$string['numberofcorrectanswers'] = '정답 수: $a';
$string['numberofcorrectmatches'] = '바른 연결을 한 갯수: $a';
$string['numberofpagestoshow'] = '보여줄 쪽(카드)수';
$string['numberofpagesviewed'] = '응답한 질문의 수: $a';
$string['ongoing'] = '현재 점수 보기';
$string['ongoingcustom'] = '당신은 $a->currenthigh (최고)점 중 $a->score 점입니다.';
$string['ongoingnormal'] = '당신은 $a->viewed 개의 질문 중  $a->correct 질문에 정확한 답을 했습니다.';
$string['or'] = '또는';
$string['ordered'] = '정렬';
$string['other'] = '다른';
$string['outof'] = '$a 중';
$string['outoftime'] = '시간에서';
$string['overview'] = '개요';
$string['page'] = '쪽: $a';
$string['pagecontents'] = '쪽의 내용';
$string['pages'] = '쪽들';
$string['pagetitle'] = '쪽 제목';
$string['password'] = '비밀번호';
$string['passwordprotectedlesson'] = '$a 는 학습에서 쓰이는 비밀번호입니다.';
$string['pleasecheckoneanswer'] = '한 개의 답을 고르십시오';
$string['pleasecheckoneormoreanswers'] = '하나 혹은 하나 이상의 답을 고르십시오.';
$string['pleaseenteryouranswerinthebox'] = '상자 안에 답을 입력하십시오.';
$string['pleasematchtheabovepairs'] = '짝을 지어 맞추십시오';
$string['pointsearned'] = '얻은 점수';
$string['postsuccess'] = '성공적으로 게시되었습니다.';
$string['practice'] = '모의 학습';
$string['preview'] = '미리보기';
$string['previewlesson'] = '$a 미리보기';
$string['previouspage'] = '앞 쪽';
$string['progressbar'] = '진척 상황 막대';
$string['progressbarteacherwarning'] = '$a 에 대해서는 진척 상황 막대가 보이지 않음';
$string['question'] = '질문';
$string['questionoption'] = '질문 옵션';
$string['questiontype'] = '질문 형태';
$string['randombranch'] = '무작위 갈래쪽';
$string['randompageinbranch'] = '갈래내의 무작위 질문';
$string['rank'] = '등위';
$string['receivedcredit'] = '이수함';
$string['redisplaypage'] = '쪽 다시 보기';
$string['report'] = '보고';
$string['reports'] = '보고서';
$string['response'] = '반응';
$string['returnmainmenu'] = '주메뉴로 돌아가기';
$string['returntocourse'] = '배움터로 돌아가기';
$string['review'] = '미리보기';
$string['reviewlesson'] = '학습 다시보기';
$string['reviewquestionback'] = '예, 다시하겠습니다.';
$string['reviewquestioncontinue'] = '아니오, 다음 질문으로 넘어가겠습니다.';
$string['sanitycheckfailed'] = '정상감시가 실패하였습니다: 이번 시도는 무효처리 됩니다.';
$string['savechanges'] = '저장하기';
$string['savechangesandeol'] = '모두 저장하고 학습 종료로 가기';
$string['savepage'] = '쪽 저장';
$string['score'] = '점수';
$string['scores'] = '점수들';
$string['secondpluswrong'] = '정확하지 않습니다. 다시 하시겠습니까?';
$string['showanunansweredpage'] = '답을 하지 않은 쪽 보기';
$string['showanunseenpage'] = '아무도 보지 않은 쪽 보기';
$string['singleanswer'] = '단답';
$string['skip'] = '진행(navigation) 생략';
$string['slideshow'] = '슬라이드쇼';
$string['slideshowbgcolor'] = '슬라이드쇼 배경';
$string['slideshowheight'] = '슬라이드쇼 높이';
$string['slideshowwidth'] = '슬라이드쇼 넓이';
$string['startlesson'] = '학습 시작하기';
$string['studentattemptlesson'] = '$a->firstname $a->lastname 의 시도 수 : $a->attempt';
$string['studentname'] = '이름 : $a';
$string['studentoneminwarning'] = '경고: 학습 종료 시간이 1분 정도 남았습니다.';
$string['studentoutoftime'] = '알림 : 학습 시간이 모두 끝났습니다. 당신의 마지막 답은 시간이 다 끝난 후에 제출됐기 때문에 적용되지 않습니다. 학습과정을 마치기 위해서 계속 버튼을 눌러주세요.';
$string['studentresponse'] = '{$a} 의 대답';
$string['submitname'] = '이름 입력';
$string['teacherjumpwarning'] = '$a->cluster 나 $a->unseen 의 건너뛰기는 이 학습과정에서만 사용됩니다. 다음 쪽 이동이 대신 사용될것입니다. 건너뛰기를 점검하기 위해서는 학생으로 로그인하세요.';
$string['teacherongoingwarning'] = '현재 점수는 오직 학생들에게만 보여집니다. 현재 점수를 확인하기 위해서는 학생으로 로그인하십시오.';
$string['teachertimerwarning'] = '타이머는 학생들을 위해서만 작동됩니다. 학생으로 로그인 하여 타이머를 점검하세요.';
$string['thatsthecorrectanswer'] = '올바른 답 입니다.';
$string['thatsthewronganswer'] = '잘못된 답 입니다.';
$string['thefollowingpagesjumptothispage'] = '다음의 쪽에서 이 페이지로 되돌아 오게 됩니다.';
$string['thispage'] = '제자리';
$string['timed'] = '시간제한';
$string['timeremaining'] = '남은 시간';
$string['timespenterror'] = '적어도 $a 분간은 학습에 임해야 합니다.';
$string['timespentminutes'] = '시간 소모(분)';
$string['timetaken'] = '시간이 걸렸음';
$string['topscorestitle'] = '상위 $a 고득점';
$string['unseenpageinbranch'] = '갈래내 보지 않은 질문';
$string['unsupportedqtype'] = '지원되지 않는 질문 형식 ($a)!';
$string['updatedpage'] = '페이지 갱신';
$string['updatefailed'] = '업데이트 실패';
$string['updatesuccess'] = '업데이트 성공';
$string['useeditor'] = '편집기 사용';
$string['usemaximum'] = '최대치 사용';
$string['usemean'] = '평균치 사용';
$string['usepassword'] = '암호로 보호된 학습';
$string['viewgrades'] = '점수 보기';
$string['viewhighscores'] = '고득점 목록 보기';
$string['viewreports'] = '$a->attempts 회 마친 $a->student 의 결과 보기';
$string['waitpostscore'] = '고득점 목록에 기록되는 중입니다. 잠시만 기다리세요.';
$string['welldone'] = '잘했어요!';
$string['whatdofirst'] = '처음에 무엇을 하길 원합니까?';
$string['wronganswerjump'] = '오답의 경우 이동할 곳';
$string['wronganswerscore'] = '오답에 대한 점수';
$string['wrongresponse'] = '오답에 대한 반응';
$string['youhavereceived'] = '이 서술형 질문에 대해 $a->outof 중 $a->score 을 받았습니다.';
$string['youhaveseen'] = '당신은 이미 이 학습을 시도한 적이 있습니다.<br /> 도중에 끝마쳤던 부분부터 시작하길 원합니까?';
$string['youmadehighscore'] = '당신은 $a 를 고득점 목록에 올렸습니다.';
$string['youranswer'] = '당신의 대답';
$string['yourcurrentgradeis'] = '현재 점수는 $a 입니다';
$string['yourgradeisnow'] = '학습에 대한 점수가 $a로 바뀌어졌습니다.';
$string['yourresponse'] = '당신의 반응';
$string['youshouldview'] = '당신은 적어도 $a 에 답해야만 합니다.';

?>
