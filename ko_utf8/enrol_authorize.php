...
$string['logininfo'] = '보안관리상 사용자 ID, 비밀번호 및 거래키는 보이지 않습니다. 이전에 이미 항목을 설정한 적이 있다면, 더 이상 입력할 필요는 없습니다. 이미 설정한 항목들은 상자의 왼페이지에 푸른 색으로 보여집니다. 만일 처음 입력하는 것이라면, 사용자 ID(*)는 물론, 필히 거래키(#1) <strong>혹은</strong> 비밀번호(#2) 중 <strong>하나</strong>를 적절한 입력상자에 써 넣어야만 합니다. 가급적 보안을 위해서는 거래키를 입력할 것을 권고합니다. 만일 현재의 비밀번호를 삭제하고자 한다면 체크박스에 체크하시면 됩니다.';
