<?PHP // $Id$ 
      // auth.php - created with Moodle 1.7 dev (2006091500)


$string['alternatelogin'] = '만약 당신이 어떤 윕주소를 통해 여기에 있다면, 그 것은 이 사이트를 위한 로그인 페이지에서 사용된 것이다. 이 페이지는 <strong>\'$a\'</strong> 처럼 실행문을 가진 형태이어여야 하고 <strong>사용자ID</strong>  <strong>password</strong> 그리고 Return 필드를 포함하여야 한다. 
<br />부정확한 URL을 입력하지 않도록 주의하지 않으면 이 사이트에 갇혀 버릴 수도 있다.<br />
...
<br />cas_baseuri = CAS/';
$string['auth_cas_create_user'] = '만약 당신이 CAS 인증을 득한 사용자를 무들 데이터베이스에 삽입하고 싶다면 이 기능을 켜기 바란다. 그렇게 하지 않으면 기존 무들 데이터베이스에 존재하는 사용자만이  로그인 할 수 있다.';
$string['auth_cas_enabled'] = 'CAS 인증을 사용하기 원한다면 이것을 켜십시오.';
$string['auth_cas_hostname'] = 'CAS 서버의 호스트네임 <br />예: host.domain.fr';
...
$string['auth_casdescription'] = '이 방법은 간편 사용자 인증(SSO)하에서 사용자를 인증하기 위해 CAS 서버(중앙 인증 서비스)를 사용한다. 또한 단순한 LDAP 인증을 사용할 수도 있다. 만약 주어진 사용자 이름과 비밀 번호가 CAS에 따라 근거가 확실하다면, 무들은 그것의 데이터베이스에서 새로운 사용자 엔트리를 만들고, 만약 요청이 있으면 LDAP로부터 사용자 속성을 가져온다. 로그인과정에서는 단지 사용자 이름과 비밀번호만 점검된다.';
$string['auth_castitle'] = 'CAS 서버(SSO) 사용';
$string['auth_changepasswordhelp'] = '비밀번호 도움문서 바꾸기';
$string['auth_changepasswordhelp_expl'] = '비밀번호 도움문서를 $a 비밀번호를 잊어버린 사용자들에게 보여줍니다. 또한 이것은 무들의 비밀번호 변경 혹은 <strong>비밀번호 변경 주소</strong>를 대신 보여줄 수도 있습니다.';
...
$string['auth_dbpasstype'] = '<p>비밀번호 필드의 포멧을 구체적으로 적어라. PostNuke와 같은 웹 프로그램으로 연결하기 위해서는 MD5 암호화를 사용하는 것이 유용하다.</p> <p>만일 사용자명이나 이메일주소는 외부 데이터베이스를 사용하지만, 무들이 암호를 관리할 때에는 \'internal\'을 사용하라. 이때 <i>반드시</i> 외부 디비의 이메일주소 필드를 확보해야 하고, auth/db/cron.php 를 주기적으로 실행시켜야만 한다. 무들은 임시 비밀번호로 새 사용자에게 이메일을 발송할 것이다.</p>';
...
$string['auth_emaildescription'] = '이메일 확인 인증은 기본 인증 방법이다. 사용자가 가입할 때, 새로운 사용자 아이디와 비밀번호를 선택하면, 사용자의 이메일 계정으로 확인 메일이 보내진다. 이 메일에는 계정을 활성화할 수 있는 안전한 링크를 포함한다. 다음 로그인할 경우에는 무들 데이타베이스에 저장된 값을 참고하게 된다.';
...
$string['auth_fcuserid'] = '\'Subadministrator\'권한 설정을 위한 FirstClass 계정의 사용자ID';
...
$string['auth_fieldlocks_help'] = '<p>당신은 사용자 정보 분야를 잠굴수 있다. 이 기능은 사용자 정보 기록이나 사용자 정보를 사이트에서 운영자가 직접 유지 관리하는데 유용하다. 무들 프로그램에 의해 잠긴 분야가 있다면 사용자 계정을 새로만들거나 하지 않으면 계정은 사용할수 없게 될 것이다.</p><p>이런 문제가 생기지 않길 바란다면 설정을 칸이 비워져있을땐 잠기지 않음으로 설정하라.</p>';
$string['auth_imapdescription'] = 'IMAP 서버를 사용하여 사용자의 이름과 패스워드가 유효한지를 확인한다.';
$string['auth_imaphost'] = 'IMAP 서버의 주소. DNS 이름을 사용하지 않고 IP주소를 사용한다.';
$string['auth_imapport'] = 'IMAP 서버의 포트 숫자를 나타낸다. 보통 이 숫자는 143이거나 993이다.';
$string['auth_imaptitle'] = 'IMAP 서버의 사용';
$string['auth_imaptype'] = 'IMAP 서버의 유형. IMAP 서버는 다른 유형의 인증방법이나 교섭방법을 사용 할 수 있다.';
$string['auth_ldap_bind_dn'] = '만약 당신이 bind-user(운영자급 사용자를 지칭한다)을 이용하여 사용자들을 찾길 바란다면 이곳에 자세한 것을 기록해야한다. 예를 들면 \'cn=ldapuser,ou=public,o=org\' 이런 것들이 있다.';
...
$string['auth_ldap_create_context'] = '만약 당신이 이메일 인증으로 사용자를 생성시키려 한다면 어디에 사용자들을 생성시킬지를 명시하시오. 이 문맥은 보안상의 문제를 막기위해 다른 사용자들과는 다르게 명기되어야 한다. ldap_context-variable에 작성된 문맥을 포함할 필요가 없다. 무들이 자동적으로 작성된 문맥에서 사용자를 찾아줄 것이다.<br /><b>주의!</b> 사용자 생성을 위해서는 auth/ldap/lib.php에 있는 auth_user_create()를 변경시킬 필요가 있다.';
