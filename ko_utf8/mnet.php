...
$string['trustedhostsexplain'] = '<p>호스트 인증 매카니즘은 무들 API의 일부분으로 소위 XML-RPC를 호출하여 특정 기계에서 이를 실행시킬 수 있로록 한다. 이는 스크립트가 무들의 행동을 관장하게 하는데 활성화 시키기에는 매우 위험한 선택 사항이 될 것이다. 만일 확신이 서지 않는다면, 이를 꺼 두도록 하라.</p>
 <p>이것은 무들 네트웍에서는 필요하지 <strong>않다</strong>.</p>
 <p>이것을 활성화시키기 위해서는, 한 줄에 하나씩 IP 주소 혹은 네트웍 목록을 입력하라.
 Some examples:</p>로칼 호스트:<br />127.0.0.1<br />로칼 호스트(네트웍 블록):<br />127.0.0.1/32<br />IP 주소가 192.168.0.7인 호스트만:<br />192.168.0.7/32<br />IP 주소가 192.168.0.1 에서 192.168.0.255 사이의 모든 호스트:<br />192.168.0.0/24<br />모든 호스트:<br />192.168.0.0/0<br />물론 마지막 예는 추천할 수 <strong>없는</strong> 사례이기는 하지만서도';
