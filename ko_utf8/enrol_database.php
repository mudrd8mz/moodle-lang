<?PHP // $Id$ 
      // enrol_database.php - created with Moodle 1.7 dev (2006091901)


$string['autocreate'] = '무들에 없는 과정에 등록을 하면 과정이 자동적으로 생성될 수 있습니다.';
$string['autocreation_settings'] = '자동 생성 설정';
$string['category'] = '자동 설정 과목 분야';
$string['course_fullname'] = '과정 명칭이 저장되는 항목 이름';
$string['course_id'] = '과정 아이디가 저장되는 항목 이름. 이 항목의 값은 무들의 과정 테이블에서 \"enrol_db_l_coursefield\" 항목의 값과 연결시키는데 사용됩니다.';
$string['course_shortname'] = '과정의 간단한 이름이 저장되는 항목 이름';
$string['course_table'] = '과정에 대한 세부 사항 (간단한 이름, 전체이름, 아이디 등)을 찾을 수 있는 테이블의 이름';
$string['description'] = '당신은 등록을 제어하기 위해 외부 데이터베이스 (거의 모든 종류)를 사용할 수 있습니다. 이 때 당신의 외부 데이터베이스는 배움터 아이디와 사용자 아이디 필드를 가지고 있다고 가정합니다. 이들은 여기에서 선택된 해당 항목과 비교됩니다.';
$string['enrolname'] = '외부 데이터베이스';
$string['general_options'] = '일반적인 선택사항';
$string['host'] = '데이터베이스 서버 호스트 이름';
$string['local_coursefield'] = '원격데이터베이스에 있는 아이디 번호등의 항목과 연결시킬때 사용하는 배움터 테이블의 필드명';
$string['local_fields_mapping'] = '무들(로컬) 데이터베이스 항목';
$string['name'] = '사용할 특별한 데이터베이스';
$string['pass'] = '서버에 접속하기 위한 암호';
$string['remote_fields_mapping'] = '등록(원격) 데이터베이스 항목';
$string['server_settings'] = '외부 데이터베이스 서버 설정';
$string['student_coursefield'] = '과정 아이디를 찾을 수 있는 학생등록테이블의 항목 이름';
$string['student_l_userfield'] = '사용자를 아이디번호등 학습자의 원격 기록과 연결시키기 위해서 사용하는 로컬사용자 테이블의 항목 이름';
$string['student_r_userfield'] = '사용자 아이디를 찾을 수 있는 원격 학습자 등록 테이블의 항목 이름';
$string['student_table'] = '학생등록이 저장되는 테이블 이름';
$string['teacher_coursefield'] = '과정 아이디를 찾을 수 있는 교수자 등록 테이블의 항목 이름';
$string['teacher_l_userfield'] = '사용자를 아이디번호등 교수자의 원격 기록과 연결시키기 위해서 사용하는 로컬사용자 테이블의 항목 이름';
$string['teacher_r_userfield'] = '사용자 아이디를 찾을 수 있는 원격 교수자 등록 테이블의 항목 이름';
$string['teacher_table'] = '교수자 등록이 저장되는 테이블 이름';
$string['template'] = '선택사항: 자동 생성 과정은 템플릿 과정에서 설정을 복사할 수 있습니다. 여기에 템플릿 과정의 간단한 이름을 기입하시오.';
$string['type'] = '데이터베이스 서버 종류';
$string['user'] = '서버에 접속하기 위한 사용자이름';

?>
