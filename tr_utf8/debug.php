<?PHP // $Id$ 
      // debug.php - created with Moodle 2.0 dev (Build: 20090318) (2009030501)


$string['authpluginnotfound'] = 'Kimlik denetimi eklentisi ($a) bulunamadı.';
$string['blocknotexist'] = '$a bloğu yok.';
$string['cannotbenull'] = '$a, boş olamaz.';
$string['cannotdowngrade'] = '$a->oldversion dan $a->newversion e indirgeme yapılamaz.';
$string['cannotfindadmin'] = 'Yönetici kullanıcı bulunamadı!';
$string['cannotinitpage'] = 'Sayfa tamamen ilklenemiyor: hatalı $a[0] id $a[1]';
$string['cannotsetuptable'] = '$a tablolaları başarıyla AYARLANAMADI!';
$string['codingerror'] = 'Kodlama hatası algılandı, bir programcı tarafından düzeltilmeli: $a';
$string['configmoodle'] = 'Moodle, henüz yapılandırılmamış. Öncelikle config.php dosyasını düzenlemeniz gerekiyor.';
$string['erroroccur'] = 'İşlem sırasında hata oluştu';
$string['fixsetting'] = 'Lütfen config.php deki ayarları düzeltin: <p>Şimdiki:</p> <p>$CFG->dirroot = \"$a->current\";</p><p>Olması gereken:</p> <p>$CFG->dirroot = \"$a->found\"</p>';
$string['modulenotexist'] = '$a eklentisi yok';
$string['noblocks'] = 'Bloklar kurulmadı!';
$string['nocate'] = 'Kategori yok!';
$string['nomodules'] = 'Eklentiler bulunamadı!';
$string['noreports'] = 'Erişilebilecek rapor yok';
$string['notables'] = 'Tablo Yok!';
$string['phpvaroff'] = 'PHP değişkeni \'$a->name\' Kapalı olmalı - $a->link';
$string['phpvaron'] = 'PHP değişkeni \'$a->name\' Açık olmalı - $a->link';
$string['siteisnotdefined'] = 'Site id tanımlanmadı!';
$string['sqlrelyonobsoletetable'] = 'Bu SQL kullanılmayan tablo(lar) üzerinde sorgu çalıştırıyor: $a! Kodunuz bir programcı tarafından düzeltilmeli.';
$string['withoutversion'] = 'Ana version.php dosyası okunabilir değil veya yok';
$string['xmlizeunavailable'] = 'xmlize fonksiyonları kullanılamıyor';
$string['cannotcreateadminuser'] = 'CİDDİ HATA: Yönetici kullanıcı kaydı oluşturulamadı.'; // ORPHANED
$string['cannotsetupsite'] = 'Ciddi hata! Site ayarlanamadı.'; // ORPHANED
$string['cannotupdaterelease'] = 'HATA: Sürüm bilgisi veritabanına yazılamadı!'; // ORPHANED
$string['cannotupdateversion'] = 'Güncelleme hatası! (Config tablosu güncellemesi yapılamadı)'; // ORPHANED
$string['cannotupgradecapabilities'] = 'Rol Sistemindeki ana yetkiler güncellenirken hata oluştu'; // ORPHANED
$string['dbnotinsert'] = 'Veritabanı hatası - ($a) eklenemedi'; // ORPHANED
$string['dbnotsetup'] = 'Hata: Ana tablolar başarıyla ayarlanamadı'; // ORPHANED
$string['dbnotsupport'] = 'Hata: Veritabanınız ($a) Moodle tarafından tam olarak desteklenmiyor veya lib/db/install.xml dosyası yok.'; // ORPHANED
$string['dbnotupdate'] = 'Veritabanı hatası - ($a) güncellenemedi'; // ORPHANED
$string['doesnotworkwitholdversion'] = 'Bu betik, Moodle\'ın eski sürümüyle çalışmaz.'; // ORPHANED
$string['upgradefail'] = 'Güncelleme hatası! bakınız: $a'; // ORPHANED

?>
