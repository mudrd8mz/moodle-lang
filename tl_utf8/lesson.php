<?PHP // $Id$ 
      // lesson.php - created with Moodle 1.9 dev (2007041800)


$string['accesscontrol'] = 'Kontrol ng pagpasok';
$string['actionaftercorrectanswer'] = 'Aksiyon pagkatapos ng wastong sagot';
$string['actions'] = 'Mga aksiyon';
$string['activitylink'] = 'Maglink sa isang aktibidad';
$string['activitylinkname'] = 'Pumunta sa: $a';
$string['addabranchtable'] = 'Magdagdag ng Manghad ng Sanga';
$string['addanendofbranch'] = 'Magdagdag ng Dulo ng Sanga';
$string['addaquestionpage'] = 'Magdagdag ng Pahina ng Tanong';
$string['addaquestionpagehere'] = 'Magdagdag ng pahina ng tanong dito';
$string['addcluster'] = 'Magdagdag ng Kumpol';
$string['addedabranchtable'] = 'Nagdagdag ng Manghad ng Sanga';
$string['addedanendofbranch'] = 'Nagdagdag ng Dulo ng Sanga';
$string['addedaquestionpage'] = 'Nagdagdag ng Pahina ng Tanong';
$string['addedcluster'] = 'Nagdagdag ng Kumpol';
$string['addedendofcluster'] = 'Nagdagdag ng Dulo ng Kumpol';
$string['addendofcluster'] = 'Magdagdag ng Dulo ng Kumpol';
$string['addpage'] = 'Magdagdag ng pahina';
$string['anchortitle'] = 'Simula ng pangunahing nilalaman';
$string['and'] = 'AT';
$string['answer'] = 'Sagot';
$string['answeredcorrectly'] = 'nasagot nang wasto.';
$string['answersfornumerical'] = 'Ang mga sagot para sa mga Denumerong tanong ay dapat magkatugmang pares ng Minimum at Maksimum na halaga';
$string['arrangebuttonshorizontally'] = 'Iaayos ba ang mga buton ng Sanga nang pahiga, sa modong slideshow?';
$string['attempt'] = 'Pagkuha: $a';
$string['attempts'] = 'Mga pagkuha';
$string['attemptsdeleted'] = 'Binurang pagkuha';
$string['attemptsremaining'] = 'May $a pagkuha kang nalalabi';
$string['available'] = 'Makukuha simula sa';
$string['averagescore'] = 'Katamtamang Iskor';
$string['averagetime'] = 'Katamtamang Oras';
$string['branchtable'] = 'Manghad ng Sanga';
$string['cancel'] = 'Balewalain';
$string['canretake'] = '$a ay maaaring kumuha-mulî';
$string['casesensitive'] = 'Gumamit ng Regular na Ekspresyon';
$string['checkbranchtable'] = 'Subukin ang manghad ng sanga';
$string['checkedthisone'] = 'tsinekan ang isang ito.';
$string['checknavigation'] = 'Subukin ang nabigasyon';
$string['checkquestion'] = 'Subukin ang tanong';
$string['classstats'] = 'Estadistika ng klase';
$string['clicktodownload'] = 'Iklik ang sumusunod na link para mailusong ang sako.';
$string['clicktopost'] = 'Iklik ito upang mapaskil ang marka mo sa listahan ng Pinakamatataas na Iskor.';
$string['clusterjump'] = 'Di pa nakikitang tanong sa loob ng isang kumpol';
$string['clustertitle'] = 'Kumpol';
$string['collapsed'] = 'Pinitpit';
$string['comments'] = 'Mga opinyon mo';
$string['completed'] = 'Nakumpleto na';
$string['completederror'] = 'Kumpletohin ang aralin';
$string['completethefollowingconditions'] = 'Kailangan mong kumpletohin ang sumusunod na kondisyon sa <b>$a</b> na aralin bago ka makapagpatuloy.';
$string['conditionsfordependency'] = 'Kondisyon para sa mga nakadepende';
$string['confirmdeletionofthispage'] = 'Kumpirmahin ang pagbura ng pahinang ito';
$string['congratulations'] = 'Binabatì ko kayo - narating ninyo na ang wakás ng aralin';
$string['continue'] = 'Ituloy';
$string['continuetoanswer'] = 'Ituloy ang pagbabago ng mga sagot.';
$string['correctanswerjump'] = 'Paglukso para sa wastong sagot';
$string['correctanswerscore'] = 'Iskor ng wastong sagot';
$string['correctresponse'] = 'Tugon para sa wasto';
$string['credit'] = 'Marka';
$string['customscoring'] = 'Pasadyang pag-iiskor';
$string['deadline'] = 'Huling araw ng pasahan';
$string['defaultessayresponse'] = 'Mamarkahan ang sanaysay mo ng gurò ng kurso';
$string['deletedefaults'] = 'Binurang $a x umiiral ng aralín';
$string['deletedpage'] = 'Binura ang pahina';
$string['deleting'] = 'Binubura';
$string['deletingpage'] = 'Binubura ang pahina: $a';
$string['dependencyon'] = 'Nakadepende sa';
$string['description'] = 'Deskripsiyon';
$string['detailedstats'] = 'Detalyadong estadistika';
$string['didnotanswerquestion'] = 'Hindi sinagot ang tanong na ito.';
$string['didnotreceivecredit'] = 'Hindi nakatanggap ng marka';
$string['displaydefaultfeedback'] = 'Ipakita ang umiiral na puna';
$string['displayhighscores'] = 'Ipakita ang matataas na marka';
$string['displayinleftmenu'] = 'Ipapakita ba sa kaliwang menu?';
$string['displayleftif'] = 'at idispley lamang kung ang $a ay may marka na mas mataas sa:';
$string['displayleftmenu'] = 'Ipakita ang kaliwang menu';
$string['displayofgrade'] = 'Pagpapakita ng marka (para sa mga mag-aaral lamang)';
$string['displayreview'] = 'Ipakita ang buton na panrebyu';
$string['displayscorewithessays'] = 'Nakakuha ka ng $a->score mula sa kabuuan na $a->tempmaxgrade para sa mga tanong na kusang minamarkahan.<br />Ang $a->essayquestions na pansanaysay na tanong mo ay mamarkahan at idaragdag<br />sa iyong huling iskor sa mga susunod na araw.<br /><br />Ang kasalukuyan mong marka, na wala pa ang pansanaysay na tanong, ay $a->score mula sa kabuuan na $a->grade';
$string['displayscorewithoutessays'] = 'Ang iskor mo ay $a->score (Mula sa kabuuan na $a->grade).';
$string['edit'] = 'Iedit';
$string['editlessonsettings'] = 'Iedit ang kaayusan ng aralíng Ito';
$string['editpagecontent'] = 'Iedit ang nilalaman ng pahinang ito';
$string['emailallgradedessays'] = 'I-email ang LAHAT<br />ng minarkahang sanaysay';
$string['emailgradedessays'] = 'Iemail ang mga minarkahang sanaysay';
$string['emailsuccess'] = 'Tagumpay ang (mga) pag-i-email';
$string['endofbranch'] = 'Dulo ng sanga';
$string['endofclustertitle'] = 'Dulo ng kumpol';
$string['endoflesson'] = 'Wakás ng aralín';
$string['enteredthis'] = 'ipinasok ito.';
$string['entername'] = 'Magpasok ng palayaw para sa listahan ng matataas na iskor';
$string['enterpassword'] = 'Magpasok ng kontrasenyas:';
$string['eolstudentoutoftime'] = 'Makinig: Naubusan ka na ng oras sa aralíng ito.  Ang huli mong sagot ay maaaring hindi naisama kung sinagot mo ito noong tapos na ang taning.';
$string['eolstudentoutoftimenoanswers'] = 'Wala kang sinagot na tanong.  Nakatanggap ka ng 0 sa aralíng ito.';
$string['essay'] = 'Sanaysay';
$string['essayemailmessage'] = '<p>Pansanaysay na Tanong:<blockquote>$a->question</blockquote></p><p>Ang iyong tugon:<blockquote><em>$a->response</em></blockquote></p><p>Opinyon ng $a->teacher :<blockquote><em>$a->comment</em></blockquote></p><p>Nakatanggap ka ng $a->earned sa kabuuang $a->outof para sa pansanaysay na tanong na ito.</p><p>Ang marka mo para sa aralín ay ginawang $a->newgrade%%.</p>';
$string['essayemailsubject'] = 'Ang marka mo para sa $a na tanong';
$string['essays'] = 'Mga sanaysay';
$string['essayscore'] = 'Iskor ng sanaysay';
$string['fileformat'] = 'Anyo ng Sako';
$string['firstanswershould'] = '<br />Ang unang sagot ay dapat lumukso sa \"Wasto\" na Pahina';
$string['firstwrong'] = 'Nakakalungkot, pero hindi ka tatanggap ng isang puntos, dahil ang tugon mo ay mali.  Nais mo bang magpatuloy sa panghuhula, para lamang sa katuwaan ng pag-aaral (pero walang puntos na matatanggap)?';
$string['flowcontrol'] = 'Kontrol ng daloy';
$string['full'] = 'Pinalapad';
$string['general'] = 'Pangkalahatan';
$string['grade'] = 'Marka';
$string['gradebetterthan'] = 'Marka na mas mataas sa (%%)';
$string['gradebetterthanerror'] = 'Tumanggap ng marka na mas mataas sa $a porsiyento';
$string['gradeessay'] = 'Markahan ang mga pansanaysay na tanong  ($a->notgradedcount hindi namarkahan at $a->notsentcount hindi ipinadala)';
$string['gradeis'] = 'Ang marka ay $a';
$string['gradeoptions'] = 'Mga opsiyon ng marka';
$string['handlingofretakes'] = 'Paano kukuwentahin ang mga pagkuha-mulî';
$string['havenotgradedyet'] = 'Hindi pa namamarkahan.';
$string['here'] = 'dito';
$string['highscore'] = 'Mataas na iskor';
$string['highscores'] = 'Matataas na Iskor';
$string['hightime'] = 'Mataas na oras';
$string['importcount'] = 'Nag-aangkat ng $a na tanong';
$string['importppt'] = 'Mag-angkat ng PowerPoint';
$string['importquestions'] = 'Mag-angkat ng mga tanong';
$string['insertedpage'] = 'Isiningit ang pahina';
$string['jump'] = 'Lumukso';
$string['jumps'] = 'Mga lukso';
$string['jumpsto'] = 'Lumukso sa <em>$a</em>';
$string['leftduringtimed'] = 'Umalis ka sa isang inorasang aralín.<br />Iklik ang \"Ituloy\" upang masimulan mulî ang aralín.';
$string['leftduringtimednoretake'] = 'Umalis ka sa isang inorasang aralín at hindi ka na<br />pinahihintulutang umulit o magpatuloy sa aralín.';
$string['lesson:edit'] = 'Iedit ang isang aktibidad na aralín';
$string['lesson:manage'] = 'Pamahalaan ang isang aktibidad na aralín';
$string['lessonclosed'] = 'Nagsara na ang aralíng ito noong $a.';
$string['lessoncloses'] = 'Pagsasara ng aralín';
$string['lessondefault'] = 'Gamitin ang kaayusan ng aralíng ito bilang umiiral';
$string['lessonformating'] = 'Anyo ng aralín';
$string['lessonmenu'] = 'Menu ng aralín';
$string['lessonnotready'] = 'Hindi pa nakahanda ang aralíng ito para makuha.  Pakikontak ang $a mo.';
$string['lessonopen'] = 'Magbubukas ang aralíng ito sa $a.';
$string['lessonopens'] = 'Pagbubukas ng aralín';
$string['lessonpagelinkingbroken'] = 'Hindi makita ang unang pahina.  Sira siguro ang pagkalink ng mga pahina ng aralín.  Kumontak po ng admin.';
$string['lessonstats'] = 'Estadistika ng aralín';
$string['linkedmedia'] = 'Nakalink na midya';
$string['loginfail'] = 'Nabigo ang paglagda, pakiulit po...';
$string['lowscore'] = 'Mababang iskor';
$string['lowtime'] = 'Mababang oras';
$string['manualgrading'] = 'Markahan ang mga Sanaysay';
$string['matchesanswer'] = 'Pagtutugma na may sagot';
$string['maxhighscores'] = 'Bilang ng ipinakitang matataas na marka';
$string['maximumnumberofanswersbranches'] = 'Maksimum na bilang ng sagot/sanga';
$string['maximumnumberofattempts'] = 'Maksimum na bilang ng pagkuha';
$string['maximumnumberofattemptsreached'] = 'Naabot na ang maksimum na bilang ng pagkuha - Pupunta na sa susunod na pahina';
$string['maxtime'] = 'Taning na oras (minuto)';
$string['maxtimewarning'] = 'May $a minuto ka upang matapos ang aralín.';
$string['mediaclose'] = 'Ipakita ang buton na pansara:';
$string['mediafile'] = 'Mag-pop-up sa sako o pahinang pangweb';
$string['mediafilepopup'] = 'Iklik ito upang makita ang sako ng midya ng araling ito.';
$string['mediaheight'] = 'Taas ng bintana:';
$string['mediawidth'] = 'lápad:';
$string['minimumnumberofquestions'] = 'Minimum na bilang ng tanong';
$string['missingname'] = 'Magpasok po ng palayaw';
$string['modattempts'] = 'Pahintulutan ang pagrebyu ng mag-aaral';
$string['modattemptsnoteacher'] = 'Ang pagrebyu ng mag-aaral ay gagana lamang para sa mga mag-aaral.';
$string['modulename'] = 'Aralín';
$string['modulenameplural'] = 'Mga aralín';
$string['movedpage'] = 'Inilipat ang pahina';
$string['movepagehere'] = 'Ilipat ang pahina dito';
$string['moving'] = 'Inililipat ang pahina: $a';
$string['multianswer'] = 'Maramingsagot';
$string['multipleanswer'] = 'Maraming Sagot';
$string['nameapproved'] = 'Tinanggap ang pangalan';
$string['namereject'] = 'Paumanhin, ang pangalan mo ay di tinanggap ng pansalâ.<br />Gumamit po ng ibang pangalan.';
$string['nextpage'] = 'Susunod na pahina';
$string['noanswer'] = 'Walang sagot na ibinigay.  Bumalik at magpasa ng sagot.';
$string['noattemptrecordsfound'] = 'Walang natagpuang rekord ng pagkuha: Walang markang ibinigay';
$string['nobranchtablefound'] = 'Walang natagpuang Manghad ng Sanga';
$string['nocommentyet'] = 'Wala pang opinyon.';
$string['nocoursemods'] = 'Walang natagpuang aktibidad';
$string['nocredit'] = 'Walang marka';
$string['noessayquestionsfound'] = 'Walang natagpuang tanong na pansanaysay sa aralíng ito.';
$string['nohighscores'] = 'Walang matataas na iskor';
$string['nolessonattempts'] = 'Walang kumukuha ng aralíng ito.';
$string['nooneansweredcorrectly'] = 'Walang nakasagot nang wasto.';
$string['nooneansweredthisquestion'] = 'Walang sumagot sa tanong na ito.';
$string['noonecheckedthis'] = 'Walang nagtsek nito.';
$string['nooneenteredthis'] = 'Walang nagpasok nito.';
$string['noonehasanswered'] = 'Wala pang sumasagot sa pansaysay na tanong.';
$string['noretake'] = 'Hindi ka pinahihintulutan na kunin mulî ang aralín.';
$string['normal'] = 'Normal - sundan ang landás ng aralín';
$string['notcompleted'] = 'Hindi pa nakukumpleto';
$string['notdefined'] = 'Walang depinisyon';
$string['nothighscore'] = 'Hindi ka nakasama sa pinakamataas na $a na listahan ng pinakamatataas na iskor.';
$string['notitle'] = 'Walang pamagat';
$string['numberofcorrectanswers'] = 'Bilang ng wastong sagot: $a';
$string['numberofcorrectmatches'] = 'Bilang ng tugmâ: $a';
$string['numberofpagestoshow'] = 'Bilang ng pahina (baraha) na ipapakita';
$string['numberofpagesviewed'] = 'Bilang ng sinagot na tanong: $a';
$string['numberofpagesviewednotice'] = 'Bilang ng sinagot na tanong: $a->nquestions; (Dapat ay sagutan mo ang hindi bababa sa: $a->minquestions)';
$string['ongoing'] = 'Ipakita ang nakukuhang kasalukuyang iskor';
$string['ongoingcustom'] = 'Nakatanggap ka na ng $a->score puntos mula sa $a->currenthigh puntos sa kasalukuyan.';
$string['ongoingnormal'] = 'Nasagot mo nang wasto ang $a->correct tanong sa $a->viewed pagkuha.';
$string['or'] = 'O';
$string['ordered'] = 'Pinagsunud-sunod';
$string['other'] = 'Iba pa';
$string['outof'] = 'Mula sa kabuuan na $a';
$string['outoftime'] = 'Ubos na ang oras';
$string['overview'] = 'Kabuuang tanaw';
$string['page'] = 'Pahina: $a';
$string['pagecontents'] = 'Nilalaman ng pahina';
$string['pages'] = 'Mga pahina';
$string['pagetitle'] = 'Pamagat ng pahina';
$string['password'] = 'Kontrasenyas';
$string['passwordprotectedlesson'] = '$a ay aralín na kailangan ng kontrasenyas.';
$string['pleasecheckoneanswer'] = 'Tsekan po ang isang sagot';
$string['pleasecheckoneormoreanswers'] = 'Tsekan po ang isa o mahigit pang sagot';
$string['pleaseenteryouranswerinthebox'] = 'Ipasok po ang sagot ninyo sa kahon';
$string['pleasematchtheabovepairs'] = 'Pagtugmain po ang mga pares sa itaas';
$string['pointsearned'] = 'Natanggap na puntos';
$string['postsuccess'] = 'Tagumpay ang pagpapaskil';
$string['practice'] = 'Pangsanay na aralín';
$string['preview'] = 'Silipin';
$string['previewlesson'] = 'Silipin ang $a';
$string['previouspage'] = 'Nakaraang pahina';
$string['progressbar'] = 'Bar ng Naisakatuparan';
$string['progressbarteacherwarning'] = 'Hindi maidispley ang Bar ng Naisakatuparan para sa $a';
$string['qtype'] = 'Urî ng pahina';
$string['question'] = 'Tanong';
$string['questionoption'] = 'Tanong';
$string['questiontype'] = 'Uri ng tanong';
$string['randombranch'] = 'Random na pansangang pahina';
$string['randompageinbranch'] = 'Random na tanong sa loob ng isang sanga';
$string['rank'] = 'Ranggo';
$string['rawgrade'] = 'Hilaw na marka';
$string['receivedcredit'] = 'Tinanggap na marka';
$string['redisplaypage'] = 'Sariwain ang pahina';
$string['report'] = 'Ulat';
$string['reports'] = 'Mga ulat';
$string['response'] = 'Tugon';
$string['returnto'] = 'Bumalik sa $a';
$string['returntocourse'] = 'Bumalik sa kurso';
$string['review'] = 'Rebyuhin';
$string['reviewlesson'] = 'Rebyuhin ang aralín';
$string['reviewquestionback'] = 'Oo, nais kong umulit';
$string['reviewquestioncontinue'] = 'Hindi, gusto ko nang pumunta sa susunod na tanong';
$string['sanitycheckfailed'] = 'Nabigo ang pagsusuri ng katinuan:  Binura na ang pagtatangkang ito';
$string['savechanges'] = 'Isilid ang mga Pagbabago';
$string['savechangesandeol'] = 'Isilid ang lahat ng pagbabago at pumunta sa wakas ng aralín.';
$string['savepage'] = 'Isilid ang pahina';
$string['score'] = 'Iskor';
$string['scores'] = 'Mga iskor';
$string['secondpluswrong'] = 'Mali pa rin.  Nais mo bang umulit?';
$string['showanunansweredpage'] = 'Ipakita ang di pa nasasagutang pahina';
$string['showanunseenpage'] = 'Ipakita ang di pa nakikitang pahina';
$string['singleanswer'] = 'Isang Sagot Lamang';
$string['skip'] = 'Lagpasan ang nabigasyon';
$string['slideshowbgcolor'] = 'Panlikurang kulay ng Slide Show';
$string['slideshowheight'] = 'Taas ng Slide Show';
$string['slideshowwidth'] = 'Lapad ng Slide Show';
$string['startlesson'] = 'Umpisahan ang aralín';
$string['studentattemptlesson'] = 'Ika  $a->attempt pagkuha ni $a->lastname, $a->firstname';
$string['studentname'] = '$a Pangalan';
$string['studentoneminwarning'] = 'Babala: May 1 minuto o kulang pa sa isang minuto para matapos ang aralín.';
$string['studentoutoftime'] = 'Makinig:  Naubusan ka na ng oras sa aralíng ito.  Hindi tinanggap ang huli mong sagot dahil ibinigay ito pagkatapos ng taning.  Pindutin po ang ituloy na buton upang matapos ang aralín.';
$string['studentresponse'] = 'tugon ni {$a}';
$string['submitname'] = 'Ipasa ang pangalan';
$string['teacherjumpwarning'] = 'Ginagamit ang $a->cluster na lukso o $a->unseen na lukso sa aralíng ito. Sa halip ay gagamitin ang Susunod-na-Pahina na lukso.  Lumagda bilang mag-aaral upang masubok ang mga luksong ito.';
$string['teacherongoingwarning'] = 'Ang kasalukuyang iskor ay ipinapakita lamang sa mga mag-aaral.  Lumagda bilang mag-aaral upang masubok ang kasalukuyang iskor';
$string['teachertimerwarning'] = 'Gumagana lamang ang orasan para sa mag-aaral.  Subukin ang orasan sa pamamagitan ng paglagda bilang mag-aaral.';
$string['thatsthecorrectanswer'] = 'Wasto ang sagot na iyan';
$string['thatsthewronganswer'] = 'Mali ang sagot na iyan';
$string['thefollowingpagesjumptothispage'] = 'Ang mga sumusunod na pahina ay lulukso sa pahinang ito';
$string['thispage'] = 'Pahinang ito';
$string['timed'] = 'Inorasan';
$string['timeremaining'] = 'Oras na nalalabi';
$string['timespenterror'] = 'Magtagal ng kahit man lamang $a minuto sa aralin';
$string['timespentminutes'] = 'Oras na Inubos (minuto)';
$string['timetaken'] = 'Oras kinuha';
$string['topscorestitle'] = 'Pinakamatataas na $a Iskor';
$string['unseenpageinbranch'] = 'Di pa nakikitang tanong sa loob ng isang sanga';
$string['unsupportedqtype'] = 'Hindi suportado ang uri ng tanong ($a)!';
$string['updatedpage'] = 'Binago ang pahina';
$string['updatefailed'] = 'Bigo ang pagbabago';
$string['useeditor'] = 'Gamitin ang editor';
$string['usemaximum'] = 'Gamitin ang maksimum';
$string['usemean'] = 'Gamitin ang katamtaman';
$string['usepassword'] = 'Aralín na may kontrasenyas';
$string['viewgrades'] = 'Tingnan ang mga marka';
$string['viewhighscores'] = 'Tingnan ang listahan ng matataas na iskor.';
$string['viewreports'] = 'Tingnan ang $a->attempts nakumpletong pagkuha ni $a->student';
$string['welldone'] = 'Magaling!';
$string['whatdofirst'] = 'Ano ang gusto mong unang gawin?';
$string['wronganswerjump'] = 'Paglukso para sa maling sagot';
$string['wronganswerscore'] = 'Iskor ng maling sagot';
$string['wrongresponse'] = 'Tugon para sa mali';
$string['youhaveseen'] = 'Nakita mo na ang mahigit sa isang pahina ng aralíng ito.<br />Gusto mo bang magsimula sa huling pahinang nakita mo?';
$string['youmadehighscore'] = 'Nakasama ka sa pinakamataas na $a sa listahan ng matataas ang iskor.';
$string['youranswer'] = 'Ang iyong sagot';
$string['yourcurrentgradeis'] = 'Ang kasalukuyan mong marka ay $a';
$string['yourcurrentgradeisoutof'] = 'Ang kasalukuyan mong marka ay $a->grade sa kabuuang $a->total';
$string['youshouldview'] = 'Dapat mong sagutin ang hindi kukulangin sa: $a';
$string['slideshow'] = 'Slide Show'; // ORPHANED
$string['editlesson'] = 'Iedit ang $a'; // ORPHANED

?>
