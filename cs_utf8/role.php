<?PHP // $Id$ 
      // role.php - created with Moodle 1.8 + (2007021503)


$string['addrole'] = 'Přidat novou roli';
$string['allow'] = 'Povolit';
$string['allowassign'] = 'Povolit přidělování rolí';
$string['allowoverride'] = 'Povolit přenastavení rolí';
$string['assignglobalroles'] = 'Přidělit globální role';
$string['assignroles'] = 'Přidělit role';
$string['blog:create'] = 'Vytvořit nové příspěvky blogu';
$string['blog:manageentries'] = 'Upravovat a spravovat položky';
$string['blog:manageofficialtags'] = 'Spravovat oficiální klíčová slova';
$string['blog:managepersonaltags'] = 'Spravovat osobní klíčová slova';
$string['blog:view'] = 'Vidět příspěvky blogu';
$string['calendar:manageentries'] = 'Spravovat všechny položky v kalendáři';
$string['calendar:manageownentries'] = 'Spravovat vlastní položky v kalendáři';
$string['capabilities'] = 'Pravomoci';
$string['capability'] = 'Pravomoc';
$string['category:create'] = 'Vytvářet kategorie';
$string['category:delete'] = 'Odstraňovat kategorie';
$string['category:update'] = 'Aktualizovat kategorie';
$string['category:visibility'] = 'Vidět skryté kategorie';
$string['course:activityvisibility'] = 'Skrýt/ukázat činnosti';
$string['course:bulkmessaging'] = 'Posílat zprávu více lidem';
$string['course:create'] = 'Vytvářet kurzy';
$string['course:delete'] = 'Odstraňovat kurzy';
$string['course:manageactivities'] = 'Spravovat činnosti';
$string['course:managefiles'] = 'Spravovat soubory';
$string['course:managegrades'] = 'Spravovat známkování';
$string['course:managegroups'] = 'Spravovat skupiny';
$string['course:managemetacourse'] = 'Spravovat metakurzy';
$string['course:managescales'] = 'Spravovat škály';
$string['course:reset'] = 'Resetovat kurz';
$string['course:sectionvisibility'] = 'Nastavovat viditelnost sekce';
$string['course:setcurrentsection'] = 'Nastavovat aktuální sekci';
$string['course:update'] = 'Aktualizovat nastavení kurzu';
$string['course:useremail'] = 'Povolovat/zakazovat emailové adresy';
$string['course:view'] = 'Vidět kurzy';
$string['course:viewcoursegrades'] = 'Vidět známky v kurzu';
$string['course:viewhiddenactivities'] = 'Vidět skryté činnosti';
$string['course:viewhiddencourses'] = 'Vidět skryté kurzy';
$string['course:viewhiddensections'] = 'Vidět skryté sekce';
$string['course:viewhiddenuserfields'] = 'Vidět skrytá uživatelská pole';
$string['course:viewparticipants'] = 'Vidět účastníky';
$string['course:viewscales'] = 'Vidět škály';
$string['course:visibility'] = 'Skrývat/Zobrazovat kurzy';
$string['currentcontext'] = 'Stávající kontext';
$string['currentrole'] = 'Stávající role';
$string['defaultrole'] = 'Výchozí role';
$string['defineroles'] = 'Definovat role';
$string['deleterolesure'] = 'Skutečně chcete smazat roli \"$a->name ($a->shortname)\"?</p><p>Tato role je teď přidělena  $a->count uživatelům.';
$string['duplicaterole'] = 'Duplikovat tuto roli';
$string['duplicaterolesure'] = 'Skutečně chcete duplikovat roli \"$a->name ($a->shortname)\"?</p>';
$string['editrole'] = 'Upravit roli';
$string['errorbadrolename'] = 'Nesprávný název role';
$string['errorbadroleshortname'] = 'Nesprávná zkratka role';
$string['errorexistsrolename'] = 'Název role už existuje';
$string['errorexistsroleshortname'] = 'Zkratka role už existuje';
$string['existingusers'] = '$a stávajících uživatelů';
$string['globalroleswarning'] = 'UPOZORNĚNÍ! Všechny role, které přidělíte na této stránce, budou platné napříč celými stránkami včetně titulní strany a všech kurzů.';
$string['inherit'] = 'Zdědit';
$string['legacy:admin'] = 'ZÁSTUPNÁ ROLE: Správce';
$string['legacy:coursecreator'] = 'ZÁSTUPNÁ ROLE: Tvůrce kurzu';
$string['legacy:editingteacher'] = 'ZÁSTUPNÁ ROLE: Učitel (s právy úprav)';
$string['legacy:guest'] = 'ZÁSTUPNÁ ROLE: Host';
$string['legacy:student'] = 'ZÁSTUPNÁ ROLE: Student';
$string['legacy:teacher'] = 'ZÁSTUPNÁ ROLE: Učitel (bez práv úprav)';
$string['legacy:user'] = 'ZÁSTUPNÁ ROLE: Přihlášený uživatel';
$string['legacytype'] = 'Typ zástupné role';
$string['listallroles'] = 'Seznam všech rolí';
$string['manageroles'] = 'Spravovat role';
$string['metaassignerror'] = 'Tuto roli nelze přidělit uživateli \"$a\", protože je nutná pravomoc spravovat metakurz.';
$string['metaunassignerror'] = 'Role uživatele  \"$a\" byla automaticky znovu přiřazena. Prosím, zrušte namísto toho její přiřazení v asociovaných kurzech.';
$string['nocapabilitiesincontext'] = 'V tomto kontextu nejsou k dispozici žádné pravomoci.';
$string['notset'] = 'Nenastaveno';
$string['overrideroles'] = 'Přenastavit role';
$string['overrides'] = 'Přenastavení';
$string['permissions'] = 'Oprávnění';
$string['potentialusers'] = '$a uživatelů k dispozici';
$string['prevent'] = 'Zamezit';
$string['prohibit'] = 'Zakázat';
$string['question:export'] = 'Exportovat otázky';
$string['question:import'] = 'Importovat otázky';
$string['question:manage'] = 'Spravovat otázky';
$string['question:managecategory'] = 'Spravovat kategorii otázek';
$string['resetrole'] = 'Resetovat na výchozí hodnoty';
$string['resetrolenolegacy'] = 'Vymazat oprávnění';
$string['resetrolesure'] = 'Skutečně chcete resetovat roli \"$a->name ($a->shortname)\" na výchozí hodnoty?</p><p>Tyto výchozí hodnoty budou převzaty ze zástupných pravomocí ($a->legacytype).';
$string['resetrolesurenolegacy'] = 'Skutečně chcete vymazat všechna oprávnění definovaná v roli \"$a->name ($a->shortname)\"?';
$string['risks'] = 'Rizika';
$string['role:assign'] = 'Přiřadit role uživatelům';
$string['role:manage'] = 'Vytvářet a spravovat role';
$string['role:override'] = 'Přenastavovat oprávnění pro ostatním';
$string['role:switchroles'] = 'Přepínat na jiné role';
$string['role:unassignself'] = 'Rušit přidělení vlastních rolí';
$string['role:viewhiddenassigns'] = 'Vidět skrytá přidělení rolí';
$string['roleassignments'] = 'Přidělení rolí';
$string['roles'] = 'Role';
$string['roletoassign'] = 'Role k přidělení';
$string['roletooverride'] = 'Role k přenastavení';
$string['selectrole'] = 'Vybrat roli';
$string['showallroles'] = 'Ukázat všechny role';
$string['site:accessallgroups'] = 'Vstupovat do všech skupin';
$string['site:approvecourse'] = 'Schvalovat vytvoření kurzu';
$string['site:backup'] = 'Zálohovat kurzy';
$string['site:config'] = 'Měnit konfiguraci stránek';
$string['site:doanything'] = 'Možnost dělat cokoli';
$string['site:doclinks'] = 'Vidět odkazy na dokumenty vně těchto stránek';
$string['site:import'] = 'Importovat obsah do kurzu';
$string['site:manageblocks'] = 'Upravovat bloky na úrovni stránek (site)';
$string['site:mnetloginfromremote'] = 'Přihlásit se ze vzdáleného Moodlu';
$string['site:mnetlogintoremote'] = 'Roamovat do vzdáleného Moodlu';
$string['site:readallmessages'] = 'Číst všechny zprávy na stránkách';
$string['site:restore'] = 'Obnovovat kurzy';
$string['site:trustcontent'] = 'Důvěřovat uloženému obsahu';
$string['site:uploadusers'] = 'Načítat nové uživatele ze souboru';
$string['site:viewfullnames'] = 'Vždy ukazovat celá jména uživatelů';
$string['site:viewparticipants'] = 'Vidět účastníky';
$string['site:viewreports'] = 'Vidět protokoly';
$string['user:create'] = 'Vytvářet uživatele';
$string['user:delete'] = 'Odstraňovat uživatele';
$string['user:editprofile'] = 'Upravovat profil uživatele';
$string['user:loginas'] = 'Přihlásit se jako jiní uživatelé';
$string['user:readuserblogs'] = 'Vidět blogy všech uživatelů';
$string['user:readuserposts'] = 'Vidět příspěvky všech uživatelů';
$string['user:update'] = 'Aktualizovat profily uživatelů';
$string['user:viewdetails'] = 'Vidět profily uživatelů';
$string['user:viewhiddendetails'] = 'Vidět skryté detaily uživatelů';
$string['user:viewuseractivitiesreport'] = 'Vidět zprávy o činnosti uživatele';
$string['user:viewusergrades'] = 'Vidět známky uživatele';
$string['viewrole'] = 'Vidět podrobnosti role';
$string['xuserswiththerole'] = 'Uživatelů s rolí \"$a->role\": $a->number';

?>
