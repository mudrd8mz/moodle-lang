<?PHP // $Id$ 
      // calendar.php - created with Moodle 1.8 + (2007021505)


$string['advancedoptions'] = '進階選項';
$string['calendar'] = '行事曆';
$string['calendarheading'] = '$a 行事曆';
$string['clickhide'] = '點選來隱藏';
$string['clickshow'] = '點選來顯示';
$string['commontasks'] = '選項';
$string['confirmeventdelete'] = '您確定要刪除這個事件?';
$string['courseevent'] = '課程事件';
$string['courseevents'] = '課程事件';
$string['dayview'] = '依日期顯示';
$string['daywithnoevents'] = '這天沒有任何事件。';
$string['default'] = '預設';
$string['deleteevent'] = '刪除事件';
$string['detailedmonthview'] = '依月份顯示細節';
$string['durationminutes'] = '期程';
$string['durationnone'] = '沒有跨越時程';
$string['durationuntil'] = '直到';
$string['editevent'] = '編輯事件';
$string['errorbeforecoursestart'] = '無法在課程開始時間前，設定事件';
$string['errorinvaliddate'] = '錯誤的日期';
$string['errorinvalidminutes'] = '以1-999之間的數字指定時間點';
$string['errorinvalidrepeats'] = '以1-99之間的數字指定事件數';
$string['errornodescription'] = '描述是必須的';
$string['errornoeventname'] = '名稱是必須的';
$string['eventdate'] = '日期';
$string['eventdescription'] = '描述';
$string['eventduration'] = '持續時間';
$string['eventendtime'] = '結束時間';
$string['eventinstanttime'] = '時間';
$string['eventkind'] = '事件類型';
$string['eventname'] = '名稱';
$string['eventnone'] = '沒有事件';
$string['eventrepeat'] = '重覆';
$string['eventsall'] = '所有事件';
$string['eventsfor'] = '$a  事件';
$string['eventsrelatedtocourses'] = '課程相關事件';
$string['eventstarttime'] = '開始時間';
$string['eventtime'] = '時間';
$string['eventview'] = '事件細節';
$string['expired'] = '到期';
$string['explain_lookahead'] = '這個偏好設定讓您決定顯示距今幾天後的即將到來事件。';
$string['explain_maxevents'] = '這個偏好設定最大可顯示的即將到來事件數目。您可以選用一個較大的數字，但顯示過多的事件可能會佔據不少的螢幕空間。';
$string['explain_persistflt'] = '啟動時，moodle會記憶您最後一次的過濾器設定，而且在您每次登入時自動還原';
$string['explain_site_timeformat'] = '您可以選擇在整個網站使用 12 或 24 小時制的時間，如果您選擇預設，時間格式會自動根據網站的設定調整，這個設定會被使用者偏好取代。';
$string['explain_startwday'] = '這個偏好設定了每月的日曆呈現方式。使用您的慣用方式來設定它們。';
$string['explain_timeformat'] = '您可以選擇12或24小時制，如果設定為\"預設\"系統將自動依語系選擇顯示格式';
$string['export'] = '匯出';
$string['exportbutton'] = '匯出';
$string['exportcalendar'] = '匯出行事曆';
$string['for'] = '於';
$string['fri'] = '週五';
$string['friday'] = '星期五';
$string['generateurlbutton'] = '取得行事曆網址';
$string['globalevent'] = '整體事件';
$string['globalevents'] = '整體事件';
$string['gotocalendar'] = '進入行事曆';
$string['groupevent'] = '群組事件';
$string['groupevents'] = '群組事件';
$string['hidden'] = '隱藏';
$string['ical'] = 'iCal';
$string['iwanttoexport'] = '匯出';
$string['manyevents'] = '$a 事件';
$string['mon'] = '週一';
$string['monday'] = '星期一';
$string['monthlyview'] = '每月的顯示';
$string['monthnext'] = '下個月';
$string['monththis'] = '這個月';
$string['newevent'] = '新事件';
$string['noupcomingevents'] = '目前沒有即將到來的事件';
$string['oneevent'] = '一個事件';
$string['pref_lookahead'] = '即將到來的事件';
$string['pref_maxevents'] = '最多即將到來的事件';
$string['pref_persistflt'] = '記憶過濾器設定';
$string['pref_startwday'] = '一星期的第一天';
$string['pref_timeformat'] = '時間顯示格式';
$string['preferences'] = '偏好';
$string['preferences_available'] = '您的偏好設定';
$string['quickdownloadcalendar'] = '快速下載或訂閱行事曆';
$string['recentupcoming'] = '最近與未來六十天';
$string['repeateditall'] = '套用設定到 $a 的重複事件';
$string['repeateditthis'] = '套用設定到此事件';
$string['repeatnone'] = '不重覆';
$string['repeatweeksl'] = '每周重覆，同時建立';
$string['repeatweeksr'] = '事件';
$string['sat'] = '週六';
$string['saturday'] = '星期六';
$string['shown'] = '顯示';
$string['spanningevents'] = '事件進行中';
$string['sun'] = '週日';
$string['sunday'] = '星期日';
$string['thu'] = '週四';
$string['thursday'] = '星期四';
$string['timeformat_12'] = '12時制(上/下午)';
$string['timeformat_24'] = '24時制';
$string['today'] = '今天';
$string['tomorrow'] = '明天';
$string['tt_deleteevent'] = '刪除事件';
$string['tt_editevent'] = '編輯事件';
$string['tt_hidecourse'] = '課程事件被顯示(請點選來隱藏)';
$string['tt_hideglobal'] = '整體事件被顯示(請點選來隱藏)';
$string['tt_hidegroups'] = '群組事件被顯示(請點選來隱藏)';
$string['tt_hideuser'] = '使用者事件被顯示(請點選來隱藏)';
$string['tt_showcourse'] = '課程事件被隱藏(請點選來顯示)';
$string['tt_showglobal'] = '整體事件被隱藏(請點選來顯示)';
$string['tt_showgroups'] = '群組事件被隱藏(請點選來顯示)';
$string['tt_showuser'] = '使用者事件被隱藏(請點選來顯示)';
$string['tue'] = '週二';
$string['tuesday'] = '星期二';
$string['typecourse'] = '課程事件';
$string['typegroup'] = '群組事件';
$string['typesite'] = '站臺事件';
$string['typeuser'] = '使用者事件';
$string['upcomingevents'] = '即將到來的事件';
$string['urlforical'] = 'iCalendar 匯出網址，用來訂閱行事曆';
$string['userevent'] = '使用者事件';
$string['userevents'] = '使用者事件';
$string['wed'] = '週三';
$string['wednesday'] = '星期三';
$string['weeknext'] = '下星期';
$string['weekthis'] = '這星期';
$string['yesterday'] = '昨天';
$string['youcandeleteallrepeats'] = '此事件是某個重複事件的一部分，您可以僅刪除此事件，或所有 $a 事件';

?>
