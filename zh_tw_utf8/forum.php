<?PHP // $Id$ 
      // forum.php - created with Moodle 1.8 + (2007021505)


$string['addanewdiscussion'] = '新增一個討論主題';
$string['addanewquestion'] = '新增問題';
$string['addanewtopic'] = '新增一個主題';
$string['advancedsearch'] = '進階搜尋';
$string['allforums'] = '所有討論區';
$string['allowchoice'] = '允許所有人選擇';
$string['allowdiscussions'] = '設定學生張貼權限';
$string['allowratings'] = '允許對張貼內容評比嗎?';
$string['allowsallsubscribe'] = '這個討論區允許每個人選擇訂閱與否';
$string['allowsdiscussions'] = '本討論區允許每個人新增討論主題。';
$string['allsubscribe'] = '訂閱所有討論區';
$string['allunsubscribe'] = '取消訂閱所有討論區';
$string['anyfile'] = '任何檔案';
$string['attachment'] = '附件';
$string['blockafter'] = '發表鎖定限制';
$string['blockperiod'] = '時間鎖定限制';
$string['blockperioddisabled'] = '不要鎖定';
$string['bynameondate'] = '由 $a->name 發表於- $a->date';
$string['cannotadddiscussion'] = '參與社個討論區的交流需要成為群組成員';
$string['cannotviewpostyet'] = '您無法在這個討論中閱讀其他學生的問題，因為您還沒發表';
$string['configcleanreadtime'] = '從\'讀取\'資料表中清空舊資料的時間';
$string['configdisplaymode'] = '若未選擇顯示，將採用預設的顯示模式';
$string['configenablerssfeeds'] = '這個功能可以設定討論區的RSS連結功能，您只要在每個討論區設定中把它打開';
$string['configlongpost'] = '任何的發表文章超過這個長度，將被認定為太長';
$string['configmanydiscussions'] = '每頁最大顯示文章筆數';
$string['configmaxbytes'] = '預設的最大附件檔案容量大小';
$string['configoldpostdays'] = '在這些天數之後的文章會被視為已閱讀';
$string['configreplytouser'] = '當討論區的文章寄出後，要不要包含張貼者的電子郵件信箱，這樣收件人可以私下回信，不必透過討論區，如果設為\"是\"，使用者依舊可以在他們的個人設定中將他們的電子郵件設為隱藏。';
$string['configshortpost'] = '任何的文章發表小於這個長度，將被認為過短，有灌水之嫌。';
$string['configtrackreadposts'] = '如果您想要追蹤每位使用者己讀/未讀狀況請設為\'是\'。';
$string['configusermarksread'] = '如果設為\'是\'，使用者必須手動將讀過的文章標記為已讀，如果設為\'否\'，被讀過的文章將會自動標記為己讀。';
$string['couldnotadd'] = '由於不明錯誤， 無法張貼您的討論內容';
$string['couldnotdeleteratings'] = '抱歉， 本討論已被評比過， 故無法刪除';
$string['couldnotdeletereplies'] = '抱歉， 本討論主題已經有人回應， 故無法刪除';
$string['couldnotupdate'] = '由於不明錯誤，無法更新您的留言';
$string['delete'] = '刪除';
$string['deleteddiscussion'] = '本討論主題已被刪除';
$string['deletedpost'] = '本留言內容已被刪除';
$string['deletedposts'] = '文章已刪除。';
$string['deletesure'] = '確定要刪除本留言嗎?';
$string['deletesureplural'] = '您確定要刪除本篇文章和它所有的回應?($a篇張貼)';
$string['digestmailheader'] = '這是每日$a->sitename 討論區的新討論摘要，要變更您的喜好設定，請到$a->userprefs';
$string['digestmailprefs'] = '您的個人資料';
$string['digestmailsubject'] = '$a :論壇摘要';
$string['digestsentusers'] = '電子郵件摘要成功寄給$a 個使用者';
$string['disallowsubscribe'] = '不允許訂閱';
$string['disallowsubscribeteacher'] = '不允許訂閱（除了教師）';
$string['discussion'] = '討論';
$string['discussionmoved'] = '這個討論區被移至 $a';
$string['discussionmovedpost'] = '這個討論已經移動到<a href=\"$a->forumhref\">$a->forumname</a>討論區的 <a href=\"$a->discusshref\">這兒</a>';
$string['discussionname'] = '討論區名稱';
$string['discussions'] = '討論';
$string['discussionsstartedby'] = '本討論由 $a 發起';
$string['discussionsstartedbyrecent'] = '最近的討論由 $a 所發起';
$string['discussthistopic'] = '討論本主題';
$string['displayend'] = '顯示結束';
$string['displayperiod'] = '顯示期間';
$string['displaystart'] = '顯示開始';
$string['eachuserforum'] = '每人僅限發表一主題';
$string['edit'] = '編輯';
$string['editedby'] = '由$a->name -在 $a->date 編輯';
$string['editing'] = '編輯中';
$string['emptymessage'] = '留言內容有誤。  若非空白便是附件檔案過大。  您的修改並未被儲存(更新)。';
$string['everyonecanchoose'] = '每人可選擇是否訂閱';
$string['everyonecannowchoose'] = '每個人現在可以選擇訂閱';
$string['everyoneisnowsubscribed'] = '每個人現在都訂閱了這個討論區';
$string['everyoneissubscribed'] = '每人都被設定訂閱本討論區';
$string['existingsubscribers'] = '目前的訂閱者';
$string['forcessubscribe'] = '這個討論區強制每個人訂閱';
$string['forcesubscribe'] = '強迫每個人都訂閱';
$string['forcesubscribeq'] = '強迫每個人都訂閱嗎?';
$string['forum'] = '討論區';
$string['forum:addnews'] = '新增新聞';
$string['forum:createattachment'] = '建立附加檔案';
$string['forum:deleteanypost'] = '刪除任何文章（任何時間）';
$string['forum:deleteownpost'] = '刪除自己的文章（在指定時間內）';
$string['forum:editanypost'] = '編輯任何文章';
$string['forum:initialsubscriptions'] = '開始訂閱';
$string['forum:managesubscriptions'] = '管理訂閱';
$string['forum:movediscussions'] = '移動文章';
$string['forum:rate'] = '文章評分';
$string['forum:replynews'] = '回覆到新聞';
$string['forum:replypost'] = '回覆到討論';
$string['forum:splitdiscussions'] = '分割主題';
$string['forum:startdiscussion'] = '開始新主題';
$string['forum:throttlingapplies'] = '套用節流功能';
$string['forum:viewanyrating'] = '檢視任意評分';
$string['forum:viewdiscussion'] = '檢視討論';
$string['forum:viewhiddentimedposts'] = '檢視隱藏的發表時間';
$string['forum:viewqandawithoutposting'] = '永遠可以看到問答文章';
$string['forum:viewrating'] = '檢視評分';
$string['forum:viewsubscribers'] = '檢視訂閱者';
$string['forumauthorhidden'] = '作者（隱藏）';
$string['forumblockingalmosttoomanyposts'] = '您達到了發表限制，您在過去 $a->blockperiod 已經發表了 $a->numposts 篇文章，限制是 $a->blockafter 篇。';
$string['forumbodyhidden'] = '您無法檢視這篇文章，也許是因為您還沒在討論中發言。';
$string['forumintro'] = '討論區簡介';
$string['forumname'] = '討論區名稱';
$string['forumposts'] = '討論區文章';
$string['forums'] = '討論區';
$string['forumsubjecthidden'] = '主旨（隱藏）';
$string['forumtracked'] = '追蹤未閱讀文章';
$string['forumtrackednot'] = '不追蹤未閱讀文章';
$string['forumtype'] = '討論區型態';
$string['generalforum'] = '一般用途的標準討論區';
$string['generalforums'] = '一般討論區';
$string['inforum'] = '在 $a 裡';
$string['intronews'] = '一般通告與新聞';
$string['introsocial'] = '一個開放可隨意聊聊的討論區';
$string['introteacher'] = '僅限教師可參與討論區';
$string['lastpost'] = '最新文章';
$string['learningforums'] = '學習討論區';
$string['mailnow'] = '現在發送';
$string['markalldread'] = '將此討論的所有文章標示為己讀';
$string['markallread'] = '將此討論區的所有討論標示為已讀';
$string['markread'] = '標記已讀';
$string['markreadbutton'] = '標記<br />已讀';
$string['markunread'] = '標記未讀';
$string['markunreadbutton'] = '標記<br />已讀';
$string['maxattachmentsize'] = '最大附件大小';
$string['maxtimehaspassed'] = '抱歉， 超過可編輯本留言 ($a) 的時限!';
$string['message'] = '訊息';
$string['missingsearchterms'] = '下面的搜尋字串將只對應此訊息中的HTML標示語言。';
$string['modeflatnewestfirst'] = '表列回應內容， 最新張貼在前';
$string['modeflatoldestfirst'] = '表列回應內容， 早先張貼在前';
$string['modenested'] = '回應訊息將往右縮排';
$string['modethreaded'] = '回應訊息以樹狀結構呈現';
$string['modulename'] = '討論區';
$string['modulenameplural'] = '討論區';
$string['more'] = '還有';
$string['movedmarker'] = '（移動了）';
$string['movethisdiscussionto'] = '搬移這個討論主題至...';
$string['namenews'] = '新聞討論區';
$string['namesocial'] = '公開討論區';
$string['nameteacher'] = '教師專屬討論區';
$string['newforumposts'] = '討論區最新張貼';
$string['nodiscussions'] = '本討論區尚未有任何留言';
$string['nodiscussionsstartedby'] = '這個使用者沒有開啟任何主題';
$string['noguestpost'] = '抱歉， 訪客身分無法留言';
$string['noguestsubscribe'] = '抱歉，訪客不允許訂閱而且不能透過郵件收到回文。';
$string['noguesttracking'] = '抱歉，訪客不能設定追蹤選項';
$string['nomorepostscontaining'] = '沒有搜尋到包含 $a  的文章';
$string['nonews'] = '目前沒有新發表的文章';
$string['noposts'] = '無任何相關留言';
$string['nopostscontaining'] = '找不到關於 $a 的留言';
$string['noquestions'] = '在這個討論區還沒出現問題';
$string['nosubscribers'] = '尚未有人訂閱本討論區';
$string['nothingnew'] = '$a 沒有新文章';
$string['notingroup'] = '抱歉，您必需是這個群組的成員才可看這個板面';
$string['notrackforum'] = '不要追蹤未讀的訊息';
$string['noviewdiscussionspermission'] = '您沒有權限檢視這個討論區的文章';
$string['nowallsubscribed'] = '已經訂閱在 $a 的所有討論區';
$string['nowallunsubscribed'] = '沒有訂閱在 $a 的所有討論區';
$string['nownotsubscribed'] = '$a->name 將<B>不會</B>收到複製 $a->forum 留言內容的電子郵件。';
$string['nownottracking'] = '$a->name 不再追 \'$a->forum\'論壇。';
$string['nowsubscribed'] = '$a->name 將<B>會</B>收到複製 $a->forum 留言內容的電子郵件。';
$string['nowtracking'] = '$a->name 追蹤 \'$a->forum\'中。';
$string['numposts'] = '$a 留言數';
$string['olderdiscussions'] = '過期討論';
$string['oldertopics'] = '過期的主題';
$string['openmode0'] = '不能新增主題或回應留言';
$string['openmode1'] = '不能新增主題， 但可以回應留言';
$string['openmode2'] = '允許新增主題或回應留言';
$string['overviewnumpostssince'] = '篇文章自上次登入後';
$string['overviewnumunread'] = '所有未閱讀文章';
$string['parent'] = '顯示上層文章';
$string['parentofthispost'] = '本留言回應的上一篇討論';
$string['postadded'] = '您的留言已成功加入討論區。<P>您有 $a 可以修改留言內容。';
$string['postincontext'] = '檢視本留言內容';
$string['postmailinfo'] = '這是張貼在 $a 網站的一篇留言。
請按以下連結以便回應此留言內容:';
$string['postmailnow'] = '<p>這篇文章將會立即法送給所有訂閱討論區的使用者。</p>';
$string['postrating1'] = '極端獨立型';
$string['postrating2'] = '折衷型';
$string['postrating3'] = '極端交流型';
$string['posts'] = '發表文章';
$string['posttoforum'] = '張貼到論壇中';
$string['postupdated'] = '已經更新您的留言';
$string['potentialsubscribers'] = '潛在的訂閱者';
$string['processingdigest'] = '處理$a 的電子郵件摘要';
$string['processingpost'] = '處理留言 $a';
$string['prune'] = '分割';
$string['prunedpost'] = '新討論已經建立';
$string['pruneheading'] = '分割討論區並且移動此文章到新討論區';
$string['qandaforum'] = '常見問答';
$string['qandanotify'] = '這是常見問答討論區，用來檢視這些問題的其他回應，您必須先發表自己的答案。';
$string['rate'] = '評比';
$string['rating'] = '評分';
$string['ratingeveryone'] = '所有人都能評比文章';
$string['ratingno'] = '尚無評比';
$string['ratingonlyteachers'] = '只有 $a 可以評比';
$string['ratingpublic'] = '$a 可以看到所有人的評比結果';
$string['ratingpublicnot'] = '$a 僅能看針對自己的評比';
$string['ratings'] = '評比';
$string['ratingssaved'] = '儲存評比結果';
$string['ratingsuse'] = '開放評比';
$string['ratingtime'] = '該時間段內限制評分';
$string['re'] = '回應:';
$string['readtherest'] = '閱覽本區其他留言';
$string['replies'] = '回覆';
$string['repliesmany'] = '至今有 $a 篇回應';
$string['repliesone'] = '至今有 $a 篇回應';
$string['reply'] = '回應';
$string['replyforum'] = '回覆文章';
$string['resetforums'] = '從這些類型的討論區移除所有文章';
$string['resetsubscriptions'] = '移除討論區訂閱';
$string['rsssubscriberssdiscussions'] = '顯示\'$a\' 討論區中的RSS連結';
$string['rsssubscriberssposts'] = '顯示 \'$a\' 張貼的RSS連結';
$string['search'] = '搜尋';
$string['searchdatefrom'] = '討論文章必須比這個新';
$string['searchdateto'] = '討論文章必須比這個舊';
$string['searchforumintro'] = '請在下方一或多個欄位輸入搜尋字串';
$string['searchforums'] = '搜尋討論區';
$string['searchfullwords'] = '這些字要被視為一個完整的字串';
$string['searchnotwords'] = '不要包括這些字';
$string['searcholderposts'] = '搜尋文章';
$string['searchphrase'] = '字串要完全符合';
$string['searchresults'] = '搜尋結果';
$string['searchsubject'] = '這些字要出現在主旨中';
$string['searchuser'] = '這個名字要符會作者';
$string['searchuserid'] = '作者在Moodle 中的ID';
$string['searchwhichforums'] = '請選擇要搜尋的討論區';
$string['searchwords'] = '這些字可以是討論的任何字元';
$string['seeallposts'] = '檢視這個使用者的所有文章';
$string['sendinratings'] = '送出最近的評比';
$string['showsubscribers'] = '顯示訂閱者';
$string['singleforum'] = '單一簡單討論主題';
$string['startedby'] = '開始於';
$string['subject'] = '主題';
$string['subscribe'] = '訂閱本討論區';
$string['subscribeall'] = '全部訂閱本討論區';
$string['subscribed'] = '訂閱';
$string['subscribenone'] = '取消所有人的訂閱';
$string['subscribers'] = '訂閱者';
$string['subscribersto'] = '訂閱 $a';
$string['subscribestart'] = '寄回討論區文章至我的電子郵件信箱';
$string['subscribestop'] = '不要寄回討論發文章至我的電子郵件信箱';
$string['subscription'] = '訂閱';
$string['subscriptions'] = '訂閱';
$string['thisforumisthrottled'] = '這個討論區在指定的期間內有文章發表數量限制，目前設定為 $a->blockperiod 發表 $a->blockafter 篇文章。';
$string['timestartenderror'] = '顯示結束日期不能夠比開始日期早';
$string['trackforum'] = '追蹤未讀的文章';
$string['tracking'] = '追蹤';
$string['trackingoff'] = '停止';
$string['trackingon'] = '啟動';
$string['trackingoptional'] = '選項';
$string['trackingtype'] = '啟動追蹤本討論區?';
$string['unread'] = '尚未閱讀';
$string['unreadposts'] = '尚未閱讀的文章';
$string['unreadpostsnumber'] = '$a篇尚未閱讀的文章';
$string['unreadpostsone'] = '1篇尚未閱讀的文章';
$string['unsubscribe'] = '取消訂閱 本討論區';
$string['unsubscribed'] = '訂閱已取消';
$string['unsubscribeshort'] = '取消訂閱';
$string['warnafter'] = '發表文章限制警告';
$string['yesforever'] = '是的，總是如此';
$string['yesinitially'] = '是的，剛開始時';
$string['youratedthis'] = '您對此的評比是';
$string['yournewquestion'] = '您的新問題';
$string['yournewtopic'] = '您的新討論主題';
$string['yourreply'] = '您的回應內容';

?>
