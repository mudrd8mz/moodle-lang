<?PHP // $Id$ 
      // qtype_multianswer.php - created with Moodle 1.9.4+ (Build: 20090218) (2007101541)


$string['addingmultianswer'] = 'Gehitu Txertatutako erantzuna (Cloze) motako galdera';
$string['correctanswer'] = 'Erantzun zuzena';
$string['correctanswerandfeedback'] = 'Erantzun zuzena eta feedbacka';
$string['decodeverifyquestiontext'] = 'Galderaren testua deskodetu eta egiaztatu';
$string['editingmultianswer'] = 'Editatu Txertatutako erantzuna (Cloze)motako galdera';
$string['layout'] = 'Diseinua';
$string['layouthorizontal'] = 'Erantzun-botoietarako lerro horizontala';
$string['layoutselectinline'] = 'Zabaltzen den menua lerroa testuan';
$string['layoutundefined'] = 'Definitu gabeko diseinua';
$string['layoutvertical'] = 'Erantzun-botoietarako zutabe bertikala';
$string['multianswer'] = 'Txertatutako erantzunak (Cloze)';
$string['multianswersummary'] = 'Mota honetako galderak oso malguak dira, baina aukera anitzeko, erantzun laburreko eta zenbakizko galdera enbotatuak sortzen dituzten kode bereziak dituen testua gehituta bakarrik sor daitezke.';
$string['nooptionsforsubquestion'] = 'Ezin da aukerarik lortu  # $a->sub galderaren zatirako (galdera->id={$a->id})';
$string['noquestions'] = 'Erantzun anitzeko \"<strong>$a</strong>\" galderak ez du galderarik';
$string['qtypenotrecognized'] = '$a galdera-mota ezezaguna da';
$string['questiondefinition'] = 'Galderaren definizioa';
$string['questionnotfound'] = 'Ezin da aurkitu #$a galdera-zatiaren galdera';
$string['questionsmissing'] = 'Ez dago balio duen galderarik, sortu gutxienez galdera bat';
$string['unknownquestiontypeofsubquestion'] = 'Galdera-mota ezezaguna: # $a->sub galdera-zatiaren $a->type';

?>
