<?PHP // $Id$ 
      // qtype_essay.php - created with Moodle 2.0 dev (Build: 20090302) (2009021800)


$string['addingessay'] = 'Creazione domanda Componimento';
$string['editingessay'] = 'Modifica domanda Componimento';
$string['essay'] = 'Componimento';
$string['essaysummary'] = 'Permette di creare domande la cui risposta è un breve componimento. Questo tipo di domanda deve essere valutata a mano.';

?>
